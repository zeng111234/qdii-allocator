...
function buildRecommendationPlan(input) {
  ...
  const signalHealth = evaluateSignalHealth(options.history || [], options.shadowHistory || []);
  let ranking;
  try {
    ranking = buildMarketRanking(options.funds || [], options.navCache || {}, options.marketTemperature, asOf, limits);
  } catch (error) {
    return {
      asOf: asOf, action: "PAUSE", dataFreshness: { status: "ERROR", error: error.message }, budget: 0,
  ...
  let action = "BUY";
  if (!liveEnabled || signalHealth.status !== "HEALTHY") action = "PAUSE";
  if (ranking.eligible.length === 0 || selected.length === 0) action = action === "PAUSE" ? "PAUSE" : "HOLD";
  if (unknownHoldings.length > 0) action = "PAUSE";
