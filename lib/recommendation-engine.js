...
function evaluateLiveAcceptance(input) {
  const metrics = input || {};
  const failures = [];
  if (Number(metrics.rollingWindows) < 12) failures.push("INSUFFICIENT_ROLLING_WINDOWS");
  if (Number(metrics.nonOverlappingWindows) < 6) failures.push("INSUFFICIENT_NON_OVERLAPPING_WINDOWS");
  if (!(Number(metrics.medianExcess12Week) > 0)) failures.push("MEDIAN_EXCESS_NOT_POSITIVE");
  if (!(Number(metrics.drawdownGapPercentagePoints) <= 2)) failures.push("DRAWDOWN_WORSE_THAN_LIMIT");
  if (Number(metrics.shadowWeeks) < 8) failures.push("INSUFFICIENT_SHADOW_WEEKS");
  if (Number(metrics.hardRiskViolations) !== 0) failures.push("HARD_RISK_VIOLATION");
  ...
  Object.values(grouped).forEach(function (groupFunds) {
    ...
    groupFunds.forEach(function (fund) {
      ...
      Object.keys(maps[fund.code]).forEach(function (date) {
        const peers = groupFunds.map(function (peer) { return maps[peer.code][date]; }).filter(Number.isFinite).sort(function (a, b) { return a - b; });
        if (peers.length < 2) return;
        const middle = Math.floor(peers.length / 2);
        const median = peers.length % 2 ? peers[middle] : (peers[middle - 1] + peers[middle]) / 2;
        deviations.push(Math.pow(maps[fund.code][date] - median, 2));
      });
...
function buildRecommendationPlan(input) {
  ...
  const exposureResult = allocationPolicy.calculateBucketExposure(holdings, options.funds || [], Number(options.cashBalance) || 0);
