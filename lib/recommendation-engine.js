...
function evaluateLiveAcceptance(input) {
  ...
  if (metrics.strategyIdReported !== true || String(metrics.strategyId || "") !== CURRENT_STRATEGY_VERSION) {
    failures.push("STRATEGY_EVIDENCE_MISMATCH");
  }
  if (metrics.historicalPurchaseAvailabilityProven !== true) {
    failures.push("HISTORICAL_PURCHASE_AVAILABILITY_UNPROVEN");
  }
...
function uniqueRecommendationHistory(history) {
  ...
  source.forEach(function (record, index) {
    if (!record) return;
    const inferredKind = record.planKind || (record.strategy === "RecommendationPlan" ||
      (record.strategyVersion === CURRENT_STRATEGY_VERSION && (record.action === "BUY" || record.action === "PAUSE"))
      ? "BASE_RESEARCH" : "LEGACY");
    const key = record.date ? "date:" + record.date + ":" + inferredKind : "undated:" + index;
...
function partitionRecommendationHistory(history) {
  ...
  uniqueRecommendationHistory(history).forEach(function (record) {
    ...
    const inferredKind = record.planKind || (record.strategy === "RecommendationPlan" ||
      record.action === "BUY" || record.action === "PAUSE" ? "BASE_RESEARCH" : "LEGACY");
    if (inferredKind !== "BASE_RESEARCH") {
      legacyHistory.push(record);
      return;
    }
...
function buildRecommendationPlan(input) {
  ...
  const exposureResult = allocationPolicy.calculateBucketExposure(
    holdings,
    options.funds || [],
    Number(options.cashBalance) || 0,
    options.navCache || {},
    asOf
  );
  const missingValuationCodes = exposureResult.missingValuationCodes || [];
  const staleValuationCodes = Array.from(new Set(holdings.filter(function (holding) {
    const valuation = exposureResult.valuationByCode[holding.code];
    return valuation && valuation.hasConfirmedPosition && valuation.valuationComplete &&
      tradingDayLag(valuation.latestNavDate, asOf) > policy.maxFreshnessLag;
  }).map(function (holding) { return holding.code; })));
  const valuationComplete = exposureResult.valuationComplete && staleValuationCodes.length === 0;
  const amounts = {};
  let totalAmount = 0;
  if (valuationComplete) {
    holdings.forEach(function (holding) {
      const valuation = exposureResult.valuationByCode[holding.code];
      amounts[holding.code] = valuation ? valuation.value : 0;
  ...
  if (!valuationComplete) pauseReasons.push("PORTFOLIO_VALUATION_INCOMPLETE");
  let ranking;
  try {
  ...
  holdings.forEach(function (holding) {
    ...
    groupAmounts[group] = (groupAmounts[group] || 0) + (Number(amounts[holding.code]) || 0);
  });
  const assessed = ranking.eligible.map(function (row) {
    const blockedBy = row.blockedBy.slice();
    const isHeld = holdings.some(function (holding) { return holding.code === row.code; });
  ...
  const bucketExposure = valuationComplete ? exposureResult.exposure : {};
  const targetGap = valuationComplete ? allocationPolicy.calculateTargetGap(bucketExposure, policy) : {};
  let currentValue = null;
  if (valuationComplete) {
    const candidateValue = options.currentValue ?? options.portfolio?.currentValue ?? exposureResult.totalValue;
    currentValue = Number.isFinite(Number(candidateValue)) ? Number(candidateValue) : null;
  }
  ...
  return {
    ...
    dataFreshness: {
      status: !valuationComplete ? "PORTFOLIO_INCOMPLETE" : (ranking.eligible.length > 0 ? "FRESH" : "UNAVAILABLE"),
    ...
    portfolioRisk: {
      ...
      totalAmount: valuationComplete ? round2(totalAmount) : null,
      valuationComplete: valuationComplete,
      missingValuationCodes: missingValuationCodes,
      staleValuationCodes: staleValuationCodes,
      unknownHoldings: unknownHoldings,
