...
function backfillFollowUp(navCache) {
  ...
  try {
    ...
    for (var i = 0; i < data.records.length; i++) {
      ...
      // 标准化日期格式
      var recDate = normalizeDateStr(rec.date);
      if (recDate !== rec.date) {
        rec.date = recDate;
        changed = true;
      }

      var allocs = rec.ranked || [];
      for (var j = 0; j < allocs.length; j++) {
        var alloc = allocs[j];
        // 只回填 null 的字段
        if (alloc.followUp5dReturn !== null && alloc.followUp10dReturn !== null) continue;

        var navs = navCache[alloc.code];
        ...
        if (alloc.followUp5dReturn === null) {
          var nav5 = navs.find(function(n) { return n.date === addDaysToDate(recDate, 7); });
          if (!nav5) {
            // 找最近的5-7天后的净值
            for (var k = 0; k < navs.length; k++) {
              var diff = daysBetween(recDate, navs[k].date);
              if (diff >= 4 && diff <= 7) { nav5 = navs[k]; break; }
            }
          }
          if (nav5) {
        ...
        if (alloc.followUp10dReturn === null) {
          var nav10 = navs.find(function(n) { return n.date === addDaysToDate(recDate, 14); });
          if (!nav10) {
            for (var k2 = 0; k2 < navs.length; k2++) {
              var diff2 = daysBetween(recDate, navs[k2].date);
              if (diff2 >= 9 && diff2 <= 14) { nav10 = navs[k2]; break; }
            }
          }
          if (nav10) {
...
module.exports = { allocateDynamic, formatDynamicResult, scoreFund, WEIGHTS, rankTopN, backfillFollowUp };