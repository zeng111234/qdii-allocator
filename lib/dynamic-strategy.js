...
async function allocateDynamic(budget, funds, config) {
  ...
  // K线数据也分批请求避免限流
  var fundDataPairs = [];
  for (var ki = 0; ki < updatedFunds.length; ki += 5) {
    var kBatch = updatedFunds.slice(ki, ki + 5);
    var kResults = await Promise.all(kBatch.map(async fund => {
    ...
    fundDataPairs = fundDataPairs.concat(kResults);
    if (ki + 5 < updatedFunds.length) await new Promise(function(r) { setTimeout(r, 500); });
  }
