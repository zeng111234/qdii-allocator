...
async function allocateDynamic(budget, funds, config) {
  ...
  try {
    ...
    if (portfolioData && portfolioData.holdings && portfolioData.holdings.length > 0) {
      const portfolioNavCache = {};
      fundDataPairs.forEach(function (pair) {
        portfolioNavCache[pair.fund.code] = pair.history;
      });
      result.portfolioValuation = applyPortfolioAwareness(result, portfolioData, portfolioNavCache);
    }
    ...
    for (let hi = 0; hi < topPicks.length; hi++) {
      ...
      if (!hasActive) {
        ...
        if (pick.indicators) {
          ...
          const sharpe = pick.indicators.sharpeRatio || 0;
          const recentReturn = pick.indicators.recent5Change || 0;
          ...
          // 稀缺溢价：只作为候选分类，限购不代表质量，还要看风险调整收益和近期波动
          else if (pick.dailyLimit <= 100 && sharpe > 1.2 && volatility < 25 && Math.abs(recentReturn) < 10) {
  ...
  // Step 7: 保存历史记录
  if (enableHistory && config.saveHistory !== false) {
 ...
 */
function applyPortfolioAwareness(result, portfolioData, navCache) {
  if (!result.ranked || result.ranked.length === 0) {
    return { valuationComplete: true, missingValuationCodes: [], valuationByCode: {} };
  }
  ...
  portfolioData.holdings.forEach(function (h) {
    const valuation = allocationPolicy.calculateHoldingValuation(h, navCache || {});
    valuationByCode[h.code] = valuation;
    if (!valuation.valuationComplete) {
      missingValuationCodes.push(h.code);
      return;
    }
  ...
  const valuationState = {
    valuationComplete: missingValuationCodes.length === 0,
    missingValuationCodes: Array.from(new Set(missingValuationCodes)),
    valuationByCode: valuationByCode
  };
  if (!valuationState.valuationComplete) return valuationState;

  if (totalValue === 0) return valuationState;
