...
async function allocateDynamic(budget, funds, config) {
  ...
  var buySignal = "GO";
  var buySignalReasons = [];
  var temp = marketTemp ? marketTemp.temperature : 50;
  var avg = budgetInfo.avgScore || 0;
  ...
  try {
    ...
    for (let hi = 0; hi < topPicks.length; hi++) {
      ...
      if (!hasActive) {
        const type =
          pick.indicators && pick.indicators.drawdown < -5
        ...
        hypothesisEngine.createHypothesis(pick.code, pick.name, type, thesis, {
          target: 10,
          stopLoss: -8,
          timeHorizon: 30
        });
...
function formatDynamicResult(result) {
  ...
  if (result.budgetInfo && result.budgetInfo.buySignal) {
    var sig = result.budgetInfo.buySignal;
    var sigEmoji = sig === "STOP" ? "🔴" : sig === "CAUTION" ? "🟡" : "🟢";
