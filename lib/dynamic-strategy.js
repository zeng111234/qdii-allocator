...
async function allocateDynamic(budget, funds, config) {
  ...
  try {
    var hypothesisEngine = require("./hypothesis-engine");
    var navCache = require("./utils").loadNavCache();
    // 为 Top5 推荐自动创建假设
    var topPicks = ranked.slice(0, 5);
    for (var hi = 0; hi < topPicks.length; hi++) {
      var pick = topPicks[hi];
      // 检查是否已有活跃假设
      var existing = hypothesisEngine.loadHypotheses();
      var hasActive = existing.hypotheses.some(function (eh) {
        return eh.fundCode === pick.code && eh.status === "active";
      });
      if (!hasActive) {
        var type = pick.indicators && pick.indicators.drawdown < -5
        ...
        var thesis = "系统推荐#" + (hi + 1) + "，分数" + round2(pick.score) +
