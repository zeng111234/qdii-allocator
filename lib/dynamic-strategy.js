...
async function allocateDynamic(budget, funds, config) {
  ...
  try {
    ...
    console.log("[新闻情绪] " + newsSentiment.items + "条新闻, 情绪=" + newsSentiment.overall + " (+" + newsSentiment.positive + "/-" + newsSentiment.negative + "/" + newsSentiment.neutral + "中性)");
  ...
  try {
    ...
    console.log("[定投] 市场温度=" + marketTemp.temperature + "(" + marketTemp.level + ") 建议倍数=" + marketTemp.multiplier + "x");
  ...
  const { batchFetch } = require("./utils");
  const fetchResults = await batchFetch(updatedFunds, async function(fund) {
  ...
  for (let ki = 0; ki < fetchResults.length; ki++) {
    ...
    if (r && r.history && r.history.length < 10) {
      fetchFailCount++;
      console.warn("[动态策略] 基金 " + r.fund.name + "(" + r.fund.code + ") 数据不足: " + r.history.length + "条记录，需要>=10");
    }
  ...
  const scored = fundDataPairs.map(({ fund, indicators }) => {
    ...
    const result = scoreFund(fund, indicators, histCtx, recentPicksMap, premiumData, externalSignals, externalSignalMaxScore, { newsSentiment: newsSentiment, valuationData: valuationData });
    if (premiumData) {
  ...
  try {
    ...
    for (let hi = 0; hi < topPicks.length; hi++) {
      ...
      if (!hasActive) {
        const type = pick.indicators && pick.indicators.drawdown < -5
          ? hypothesisEngine.HYPOTHESIS_TYPES.MEAN_REVERSION
          : (pick.dailyLimit <= 100
            ? hypothesisEngine.HYPOTHESIS_TYPES.SCARCITY_PREMIUM
            : hypothesisEngine.HYPOTHESIS_TYPES.TREND_FOLLOWING);
        const thesis = "系统推荐#" + (hi + 1) + "，分数" + round2(pick.score) +
          (pick.reason ? "。" + pick.reason.substring(0, 80) : "");
        hypothesisEngine.createHypothesis(
          pick.code, pick.name, type, thesis,
          { target: 10, stopLoss: -8, timeHorizon: 30 }
        );
      }
    }
    // 更新已有假设的追踪收益
...
function formatDynamicResult(result) {
  ...
  if (result.marketTemperature) {
    ...
    lines.push(tempEmoji + " 市场温度: " + mt.temperature + "/100 (" + mt.level + ") 建议投入: " + adjustedBudget + "元 (" + mt.multiplier + "x)");
...
function applyPortfolioAwareness(result, portfolioData) {
  ...
  result.ranked.forEach(function(f) {
    ...
    // [fix] 加强集中度惩罚：单一类别>60%扣3分，>40%扣2分
    if (catWeight > 60) {
      penalty += 3 + (catWeight - 60) * 0.15;
      f.reason = (f.reason || "") + " [" + category + "已占" + round1(catWeight) + "%严重超配]";
    } else if (catWeight > 40) {
      penalty += 2 + (catWeight - 40) * 0.1;
