...
function applyPortfolioAwareness(result, portfolioData) {
  ...
  result.ranked.forEach(function(f) {
    ...
    if (heldValue > 0) {
      ...
      if (heldWeight > 10) {
        penalty += 5 + (heldWeight - 10) * 0.3;
        f.reason = (f.reason || "") + " [已持有" + round1(heldWeight) + "%]";
      } else if (heldWeight > 3) {
        penalty += (heldWeight - 3) * 0.15;
      }
    }
    ...
    if (catHeldCount >= 3) penalty += 3;
    else if (catHeldCount >= 2) penalty += 1.5;
...
function allocateByScore(budget, ranked, minPurchase) {
  ...
  var TYPE_CAP_RATIO = 0.4; // 单类型最多占预算40%
