...
/**
 * 持仓感知：如果已有持仓中某类型占比过高，降低该类型基金的评分
...
function applyPortfolioAwareness(result, portfolioData) {
  ...
  portfolioData.holdings.forEach(function(h) {
    ...
    // 从funds.json获取类型
    var fundType = h.type || "未知";
    typeValues[fundType] = (typeValues[fundType] || 0) + value;
  });
  ...
  result.ranked.forEach(function(f) {
    ...
    // 1. 同类型占比过高 → 降分
    var typeWeight = ((typeValues[f.type] || 0) / totalValue * 100);
    if (typeWeight > 40) {
      penalty += (typeWeight - 40) * 0.15; // 超过40%每多1%扣0.15分
      f.reason = (f.reason || "") + " [持仓同类占比" + round1(typeWeight) + "%]";
    }

    // 2. 已持有该基金 → 降分（持有金额越大扣越多）
    var heldValue = codeValues[f.code] || 0;
    if (heldValue > 0) {
      var heldWeight = heldValue / totalValue * 100;
      if (heldWeight > 15) {
        penalty += (heldWeight - 15) * 0.2; // 超过15%每多1%扣0.2分
        f.reason = (f.reason || "") + " [已持有" + round1(heldWeight) + "%]";
      } else if (heldWeight > 5) {
        penalty += (heldWeight - 5) * 0.05; // 5-15%轻度扣分
      }
    }
...
function allocateByScore(budget, ranked, minPurchase) {
  ...
  // 按分数加权（使用softmax风格分配，分数差距越大分配越不均匀）
  var totalScore = 0;
  eligible.forEach(function(f) {
    // 分数归一化：最低分设为基准，额外分数才参与加权
    var baseScore = Math.min(eligible[0].score * 0.5, 5); // 基准分
    f._allocWeight = Math.max(f.score - baseScore, 0.5); // 至少0.5权重
  ...
  eligible.forEach(function(f) {
    ...
    var alloc = Math.min(rawAlloc, cap, remaining);
    // 取整到元
    alloc = Math.floor(alloc);
    if (alloc < minPurchase) alloc = 0; // 不够最低购买额就不买
  ...
  if (remaining >= minPurchase) {
    ...
    for (var i = 0; i < allocations.length && remaining >= minPurchase; i++) {
      ...
      var canAdd = Math.min(cap - a.allocated, remaining);
