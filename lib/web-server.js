  ...
  <div class="container">
    ...
    <div class="card">
      <div class="tabs">
        ...
        <div class="tab" onclick="switchTab('daily')">每日记录</div>
      </div>
      ...
      <div id="tab-daily" class="tab-content">
        <div class="form-row" style="align-items:center">
          ...
          <div style="display:flex;gap:6px;margin-top:18px">
            <button class="btn btn-danger-sm" style="background:#f0f4ff;color:#6366f1;border-color:#c7d2fe" onclick="dailyPrev()">◀ 前一天</button>
            <button class="btn btn-danger-sm" style="background:#f0f4ff;color:#6366f1;border-color:#c7d2fe" onclick="dailyToday()">今天</button>
            <button class="btn btn-danger-sm" style="background:#f0f4ff;color:#6366f1;border-color:#c7d2fe" onclick="dailyNext()">后一天 ▶</button>
          </div>
        </div>
        <div id="dailySummary" style="margin-top:8px"></div>
        <div id="dailyList" style="margin-top:12px"></div>
        <div id="dailyTotal" style="margin-top:12px;padding-top:12px;border-top:1px solid #e2e8f0"></div>
      </div>
    </div>
  ...
  <script>
    ...
    function switchTab(tab) {
      ...
      if (tab === 'single') {
        ...
        if (!document.getElementById('dailyDate').value) {
    ...
    function dailyPrev() {
      ...
      if (!d) d = fmtDate(new Date());
      var parts = d.split('-');
      var dt = new Date(parseInt(parts[0]), parseInt(parts[1])-1, parseInt(parts[2]));
      dt.setDate(dt.getDate() - 1);
      document.getElementById('dailyDate').value = fmtDate(dt);
      loadDaily();
    }
    function dailyNext() {
      var d = document.getElementById('dailyDate').value;
    ...
    function dailyToday() {
      document.getElementById('dailyDate').value = fmtDate(new Date());
      loadDaily();
    }
    ...
    function loadDaily() {
      ...
      fetch("/api/buys").then(function(r) { return r.json(); }).then(function(data) {
        ...
        if (data.empty) {
          summaryEl.innerHTML = '<div class="empty-state">暂无持仓</div>';
          listEl.innerHTML = '';
          totalEl.innerHTML = '';
          return;
        }
        ...
        if (dayBuys.length === 0) {
          summaryEl.innerHTML = '<div class="empty-state">📅 ' + date + ' 没有买入记录</div>';
          listEl.innerHTML = '';
          totalEl.innerHTML = '';
          return;
        }
        ...
        // 按基金分组统计
        var fundMap = {};
        dayBuys.forEach(function(b) {
          if (!fundMap[b.code]) fundMap[b.code] = { name: b.name, code: b.code, buys: [], total: 0 };
          fundMap[b.code].buys.push(b);
          fundMap[b.code].total += b.amount;
        });

        summaryEl.innerHTML = '<div style="font-size:14px;font-weight:600;color:#1e293b">📅 ' + date + ' 买入 ' + dayBuys.length + ' 笔，共 ' + dayTotal + '元</div>';

        var html = '';
        Object.keys(fundMap).forEach(function(code) {
          var f = fundMap[code];
          var pct = (f.total / dayTotal * 100).toFixed(0);
          ...
          html += '<div>';
          html += '<div style="font-weight:600;font-size:14px">' + f.name + '</div>';
          html += '<div style="font-size:12px;color:#888">' + f.code + ' · ' + f.buys.length + '笔</div>';
          html += '</div>';
          html += '<div style="text-align:right">';
          html += '<div style="font-weight:700;font-size:16px;color:#6366f1">' + f.total + '元</div>';
          html += '<div style="font-size:11px;color:#aaa">' + pct + '%</div>';
          html += '</div>';
          html += '</div>';
        });
        ...
        // 每日记录的删除按钮
        totalEl.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center">' +
          '<div style="font-size:13px;color:#64748b">合计 <b style="color:#1e293b">' + dayTotal + '元</b></div>' +
          '<button class="btn btn-danger-sm" onclick="deleteDayBuys(\\'' + date + '\\')">删除当日所有记录</button>' +
          '</div>';
      });
    }

    function deleteDayBuys(date) {
      if (!confirm("确认删除 " + date + " 的所有买入记录？")) return;
      fetch("/api/buys/date/" + date, { method: "DELETE" })
        .then(function(r) { return r.json(); })
        .then(function(data) {
          if (data.error) showToast(data.error, true);
          else { showToast("已删除 " + date + " 的 " + data.deleted + " 笔记录"); loadDaily(); loadPortfolio(); }
        });
    }
          ...
          h.buyDetails.forEach(function(b, idx) {
            ...
            if (b.settled) {
              ...
              if (b.settleDate && b.settleDate <= today) {
                // 显示最新净值日期，让用户知道API延迟
                var latestInfo = h.latestDate ? ' (最新净值到' + h.latestDate + ')' : '';
                detail += ' <span style="color:#ef4444">待更新净值</span>' + latestInfo;
              } else {
...
/**
 * 从天天基金获取实时估值（东方财富官方净值更新慢时的后备）
 */
function fetchEstimatedNav(code) {
  return new Promise(function(resolve) {
    var https = require("https");
    var url = "https://fundgz.1234567.com.cn/js/" + code + ".js";
    https.get(url, { 
  ...
  <script>
      ...
      fetch("/api/quick-add", {
        ...
        if (data.error) {
          ...
          data.results.forEach(function(r) {
            if (r.ok) {
              var s = '<div class="detail">' + r.name + '</div>';
              s += r.amount + '元';
    ...
    function loadSummary() {
      fetch("/api/buys").then(function(r) { return r.json(); }).then(function(data) {
        ...
        var s = data.summary;
        var pnl = s.totalPnl >= 0 ? '+' + s.totalPnl : s.totalPnl;
        var rate = s.totalPnlRate >= 0 ? '+' + s.totalPnlRate : s.totalPnlRate;
...
function createApp() {
  ...
  app.post("/api/refresh-nav", async function(req, res) {
    var p = portfolio.loadPortfolio();
    var navCache = fundData.loadNavCache();
    var updated = 0;
    ...
    for (var h of p.holdings) {
      ...
      var cache = fundData.loadNavCache();
      var fundNavs = cache[h.code] || [];

      // 获取实时估值（东方财富更新慢时的后备）
      var estimatedNav = null;
      var unsettled = h.buys.filter(function(b) { return !b.nav && b.settleDate; });
      ...
      for (var b of h.buys) {
        if (!b.nav && b.settleDate) {
          ...
          if (navData) {
            ...
            var latestNav = fundNavs[fundNavs.length - 1].nav;
            b.nav = latestNav;
            b.shares = Math.round(b.amount / b.nav * 10000) / 10000;
            b.estimated = true;
            updated++;
  ...
  app.post("/api/quick-add", function(req, res) {
    ...
    for (var i = 0; i < entries.length; i++) {
      ...
      var holding = portfolio.recordBuy(parsed.code, parsed.name, parsed.amount, parsed.nav, parsed.date, parsed.settleDays);
      if (holding) {
