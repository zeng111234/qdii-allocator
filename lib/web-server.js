...
function buildHtml(fundsList) {
  ...
  fundsList.forEach(function(f) {
    // 提取基金公司名：匹配已知公司名
    let corp = '';
    const m = f.name.match(/^(广发|易方达|华泰柏瑞|华夏|南方|华安|嘉实|大成|博时|工银|招商|天弘|富国|银华|万家|浦银安盛|摩根|景顺长城|国泰|汇添富|鹏华|建信|中银|交银|兴全|睿远|中欧|诺安|长城|融通|泰达宏利|信诚|海富通|上投摩根|长盛|国富|光大保德信|中邮|东方|新华|民生加银|兴业|安信|宝盈|创金合信|前海开源|永赢|太平|财通|中金|红土创新)/);
    if (m) corp = m[1];
  ...
  // 生成 optgroup
  const fundOptions = Object.keys(grouped).sort().map(function(corp) {
    const options = grouped[corp].map(function(f) {
...
function createApp() {
  ...
  app.post("/api/fund-deep-dive", async function(req, res) {
    try {
      ...
      res.status(500).json({ error: "基金深度分析失败: " + error.message });
    }
  });
  ...
  app.post("/api/llm-proxy", async function(req, res) {
    try {
      ...
      const targetUrl = (baseUrl || process.env.LLM_BASE_URL || 'https://api.siliconflow.cn/v1') + '/chat/completions';
      ...
      res.status(500).json({ error: "LLM代理失败: " + error.message });
    }
  });
