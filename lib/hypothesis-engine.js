  ...
  try {
    if (fs.existsSync(HYPOTHESIS_FILE)) {
      ...
      data.stats = {
        ...
        validated: data.hypotheses.filter(h => h.status === STATUS.VALIDATED).length,
        invalidated: data.hypotheses.filter(h => h.status === STATUS.INVALIDATED).length,
        expired: data.hypotheses.filter(h => h.status === STATUS.EXPIRED).length
      };
      return data;
    }
  } catch (e) {}
  return { hypotheses: [], stats: { total: 0, active: 0, validated: 0, invalidated: 0, expired: 0 } };
}
...
function updateHypothesisReturns(navCache) {
  ...
  for (const h of data.hypotheses) {
    ...
    for (const key in periods) {
      ...
      const targetNav = fundNavs.find(function (n) {
...
function formatHypothesisReport() {
  ...
  if (stats.winRate !== null) {
    lines.push(
      "   胜率: " + stats.winRate + "% | 平均收益: " + stats.avgReturn + "% | 平均持仓: " + stats.avgHoldingDays + "天"
    );
  }
  lines.push("");
