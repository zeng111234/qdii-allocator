...
function updateHypothesisReturns(navCache) {
  ...
  for (const h of data.hypotheses) {
    ...
    for (const key in periods) {
      ...
      let targetDateStr = targetDate.toISOString().substring(0, 10);
      const targetNav = fundNavs.find(function (n) { return n.date >= targetDateStr; });
...
function formatHypothesisReport() {
  ...
  // 活跃假设
  const active = data.hypotheses.filter(function (h) { return h.status === STATUS.ACTIVE; });
  if (active.length > 0) {
    lines.push("--- 活跃假设 ---");
