...
function loadHypotheses() {
  try {
    if (fs.existsSync(HYPOTHESIS_FILE)) {
      return JSON.parse(fs.readFileSync(HYPOTHESIS_FILE, "utf-8"));
    }
  } catch (e) {}
 ...
 * 创建新假设
 */
function createHypothesis(fundCode, fundName, type, thesis, conditions) {
  ...
  const h = {
    ...
    navAtCreation: null,               // 创建时的净值
    validatedAt: null,
    invalidatedAt: null,
    outcome: null,                     // 最终结果 { return, holdingDays }
    followUpReturns: {                 // 后续追踪收益
      "3d": null,
      "7d": null,
      "14d": null,
      "30d": null
...
function updateHypothesisReturns(navCache) {
  ...
  for (const h of data.hypotheses) {
    ...
    const navAtCreation = fundNavs.find(function (n) { return n.date >= createdDate; });
    ...
    // 计算各时段收益
    var periods = { "3d": 3, "7d": 7, "14d": 14, "30d": 30 };
    for (var key in periods) {
      var targetDate = new Date(createdDate);
      targetDate.setDate(targetDate.getDate() + periods[key]);
      var targetDateStr = targetDate.toISOString().substring(0, 10);
      var targetNav = fundNavs.find(function (n) { return n.date >= targetDateStr; });
    ...
    // 检查验证/否定条件
    if (h.conditions) {
      var currentReturn = round2(((latestNav - h.navAtCreation) / h.navAtCreation) * 100);
...
function getHypothesisStats() {
  ...
  if (closed.length === 0) {
    return {
      ...
      active: data.hypotheses.filter(function (h) { return h.status === STATUS.ACTIVE; }).length,
      validated: 0,
      invalidated: 0,
      expired: 0,
      winRate: null,
  ...
  const validated = closed.filter(function (h) { return h.status === STATUS.VALIDATED; });
  const withOutcome = closed.filter(function (h) { return h.outcome; });
  ...
  return {
    ...
    avgReturn: withOutcome.length > 0
      ? round2(withOutcome.reduce(function (s, h) { return s + h.outcome.return; }, 0) / withOutcome.length)
      : null,
    avgHoldingDays: withOutcome.length > 0
      ? Math.round(withOutcome.reduce(function (s, h) { return s + h.outcome.holdingDays; }, 0) / withOutcome.length)
      : null
  };
}
...
function formatHypothesisReport() {
  ...
  lines.push("   活跃: " + stats.active + " | 验证: " + stats.validated + " | 否定: " + stats.invalidated + " | 过期: " + stats.expired);
  if (stats.winRate !== null) {
    lines.push("   胜率: " + stats.winRate + "% | 平均收益: " + stats.avgReturn + "% | 平均持仓: " + stats.avgHoldingDays + "天");
  }
  lines.push("");

  // 活跃假设
  var active = data.hypotheses.filter(function (h) { return h.status === STATUS.ACTIVE; });
  if (active.length > 0) {
    lines.push("--- 活跃假设 ---");
    for (var i = 0; i < active.length; i++) {
      var h = active[i];
      var ret3d = h.followUpReturns["3d"] !== null ? h.followUpReturns["3d"] + "%" : "N/A";
      var ret7d = h.followUpReturns["7d"] !== null ? h.followUpReturns["7d"] + "%" : "N/A";
  ...
  // 已验证/否定的假设
  var closed = data.hypotheses.filter(function (h) {
    return h.status === STATUS.VALIDATED || h.status === STATUS.INVALIDATED;
  });
  if (closed.length > 0) {
    lines.push("--- 历史假设（最近10条）---");
    var recent = closed.slice(-10);
    for (var j = 0; j < recent.length; j++) {
      var ch = recent[j];
      var emoji = ch.status === STATUS.VALIDATED ? "✅" : "❌";
      var ret = ch.outcome ? ch.outcome.return + "%" : "N/A";
  ...
  // 按类型统计
  var byType = {};
  for (var k = 0; k < data.hypotheses.length; k++) {
    var th = data.hypotheses[k];
  ...
  if (typeKeys.length > 0) {
    ...
    for (var ti = 0; ti < typeKeys.length; ti++) {
      var typeName = typeKeys[ti];
      var tStats = byType[typeName];
      var tWinRate = tStats.total > 0 ? round2((tStats.validated / tStats.total) * 100) : 0;
