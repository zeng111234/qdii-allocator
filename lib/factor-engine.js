...
function tsStd(arr, n) {
  ...
  const mean = slice.reduce(function(s, v) { return s + v; }, 0) / n;
  const sumSq = slice.reduce(function(s, v) { return s + (v - mean) * (v - mean); }, 0);
...
function computeTrendStrength(navArr) {
  ...
  const avgGain = gains.slice(-14).reduce(function(s, v) { return s + v; }, 0) / 14;
  const avgLoss = losses.slice(-14).reduce(function(s, v) { return s + v; }, 0) / 14;
...
function computeRankings(funds, navCache, marketTemperature, portfolioData) {
  ...
  funds.forEach(function(fund) {
    ...
    const navArr = sorted.map(function(x) { return x.nav; });
  ...
  results.forEach(function(r) {
    ...
    const fund = funds.find(function(f) { return f.code === r.code; });
