...
function computeAll(navArr, fundMeta, marketTemperature, _newsData, _externalSignalsData) {
  ...
  // 8. 费率评估
  scores.fee_efficiency = computeFeeEfficiency(fundMeta);
...
function computeFeeEfficiency(fundMeta) {
  if (!fundMeta) return 50;
...
function weightedSum(factorScores, weights) {
  ...
  Object.keys(factorScores).forEach(function (fid) {
    ...
    if (!Object.prototype.hasOwnProperty.call(weights, fid)) return;
    const w = weights[fid];
    if (w === 0) return;
...
module.exports = {
  ...
  computeFeeEfficiency: computeFeeEfficiency,
