...
function getAlternatives(type, excludeCode) {
  let alternatives = ALTERNATIVES_MAP[type] || [];
  if (excludeCode) {
...
function analyzeAlternatives(suspendedFunds) {
  ...
  const suggestions = [];
  for (let i = 0; i < suspendedFunds.length; i++) {
    const fund = suspendedFunds[i];
    const type = fund.type || "未知";
    const alternatives = getAlternatives(type, fund.code);
    const policyRisk = getPolicyRisk(type);
...
function formatAlternativesReport(suspendedFunds) {
  const suggestions = analyzeAlternatives(suspendedFunds);
  ...
  for (let i = 0; i < suggestions.length; i++) {
    ...
    lines.push("  \u66ff\u4ee3\u57fa\u91d1:");
    for (let j = 0; j < s.alternatives.length; j++) {
      const alt = s.alternatives[j];
...
function generatePolicyRiskOverview(funds) {
  ...
  for (let t = 0; t < types.length; t++) {
    const group = typeGroups[types[t]];
    const risk = getPolicyRisk(types[t]);
    const status = group.suspended > 0 ? "\u26a0\ufe0f" + group.suspended + "\u53ea\u505c\u8d2d" : "\u2705\u6b63\u5e38";
