 ...
 * - 100次/秒 并发
 *
 * 数据流：所有端（CLI、Web UI、GitHub Pages、GitHub Actions）通过此模块读写同一个 Firebase
 */
...
async function loadPortfolioFromFirebase() {
  return await firebaseGet("/portfolio.json");
}
...
async function savePortfolioToFirebase(portfolio) {
  return await firebasePut("/portfolio.json", portfolio);
}
