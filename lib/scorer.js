...
const WEIGHTS = {
  ...
  yearReturn: 0.02, // 1年收益率（从0.03降到0.02，进一步减少追涨）
  ...
  // 过热回调因子（降低门槛，加大惩罚）
  overheat30d: -1.2, // 30日过热扣分（从-0.8再收紧）
  overheat60d: -0.9, // 60日过热扣分（从-0.6再收紧）
  overheat90d: -0.6, // 90日过热扣分（从-0.4再收紧）

  // 辅助因子
  historicalSuccess: 2.0, // 历史推荐成功率（加倍，让验证通过的策略更受重视）
  feePenalty: -0.8, // 费率惩罚
  ...
  // 高点距离惩罚（防止追高）
  peakPenalty: -4.0 // 距离历史高点<5%时扣分（从-3.0加到-4.0）
};
...
function scoreFund(
  ...
  // ========== 过热回调因子（[fix] 收紧阈值：30日>10%, 60日>20%, 90日>30%） ==========
  if (indicators.recent30Change && indicators.recent30Change > 10) {
    const overheat30 = (indicators.recent30Change - 10) * WEIGHTS.overheat30d;
    score += overheat30;
    if (overheat30 < -0.5)
      reasons.push("30日涨" + round1(indicators.recent30Change) + "%过热" + round1(overheat30) + "分");
  }
  if (indicators.recent60Change && indicators.recent60Change > 20) {
    const overheat60 = (indicators.recent60Change - 20) * WEIGHTS.overheat60d;
    score += overheat60;
    if (overheat60 < -0.5)
      reasons.push("60日涨" + round1(indicators.recent60Change) + "%过热" + round1(overheat60) + "分");
  }
  if (indicators.recent90Change && indicators.recent90Change > 30) {
    const overheat90 = (indicators.recent90Change - 30) * WEIGHTS.overheat90d;
    score += overheat90;
    if (overheat90 < -0.5)
...
const MAX_PER_CORRELATION_GROUP = 4; // 相关性分组内最多4只（从5收紧到4）
