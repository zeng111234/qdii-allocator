...
function scoreFund(
  ...
  var drawdownScore;
  var dd = indicators.drawdown;
  if (dd > -3) {
    drawdownScore = -1.5;
  } else if (dd > -15) {
    drawdownScore = Math.min(3.0, (Math.abs(dd) - 3) * 0.25);
  } else if (dd > -25) {
    drawdownScore = 3.0 - (Math.abs(dd) - 15) * 0.15;
  } else {
    drawdownScore = Math.max(-1.0, 1.5 - (Math.abs(dd) - 25) * 0.15);
  }
  score += drawdownScore;
