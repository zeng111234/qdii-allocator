...
const WEIGHTS = {
  base: 10,
  // 长期因子（核心，但权重平衡）
  yearReturn: 0.06,         // 1年收益率（从0.1降到0.06，避免追涨历史赢家）
  threeYearReturn: 0.2,     // 3年累计收益（从0.15提到0.2，长期验证更重要）
  sharpeRatio: 2.5,         // 夏普比率（从4.5降到2.5，避免单一因子统治）
  ...
  // 中期因子
  drawdown: 3.0,            // 近期回撤（加大权重，鼓励买跌不买涨）
  maDeviation: 2.5,         // MA10偏离（从1.5提高到2.5，惩罚追高）
...
function scoreFund(fund, indicators, historyContext, recentPicks, premiumInfo, externalSignals, externalSignalMaxScore, options) {
  ...
  // 回撤因子（回撤越小越好：-10%以内加分，超过-10%扣分）
  const drawdownScore = (10 + indicators.drawdown) * WEIGHTS.drawdown * 0.1;
  score += drawdownScore;
  ...
  if (indicators.recent60Change && indicators.recent60Change > 18) {
    const overheat60 = (indicators.recent60Change - 18) * WEIGHTS.overheat60d;
    score += overheat60;
