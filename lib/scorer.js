...
const WEIGHTS = {
  base: 10,
  // 长期因子（v2: 大幅降低追涨权重，避免历史赢家统治排名）
  yearReturn: 0.005, // 1年收益率（从0.02降到0.005，彻底削弱追涨）
  threeYearReturn: 0.02, // 3年累计收益（从0.1降到0.02，历史赢家不再统治）
  sharpeRatio: 1.0, // 夏普比率（从1.5降到1.0，避免单一因子主导）
  maxDrawdown: 0.3, // 最大回撤惩罚（从0.2提到0.3，更重视风险控制）
  longTermBull: 1.5, // 长期牛市趋势加分（从2降到1.5，减少滞后信号）
  longTermBear: -3.0, // 长期熊市趋势扣分
  stabilityBonus: 3.0, // 稳定收益加分（从2.5提到3.0，更重视稳定性）

  // 中期因子（v2: 降低接飞刀风险，加强逆向信号）
  drawdown: 1.5, // 近期回撤（从3.0降到1.5，减少接飞刀风险）
  maDeviation: 4.0, // MA10偏离（从2.5提到4.0，加强逆向信号）
  trendBonus: 1.5, // 多头排列加分（从2.0降到1.5）
  trendPenalty: -2.5, // 空头排列扣分
  momentumReversal: 3.0, // 连续下跌后阳线反弹（从2.0提到3.0，加强反转信号）

  // 短期因子
  recent5Change: -0.5, // 5日涨跌（从-0.3提到-0.5，更惩罚短期追涨）
  volatility: -1.2, // 波动率惩罚（从-0.8提到-1.2，更重视稳定性）

  // 过热回调因子（v2: 收紧阈值，加大惩罚）
  overheat30d: -1.5, // 30日过热扣分（从-1.2再收紧）
  overheat60d: -1.2, // 60日过热扣分（从-0.9再收紧）
  overheat90d: -0.8, // 90日过热扣分（从-0.6再收紧）

  // 辅助因子
  historicalSuccess: 2.5, // 历史推荐成功率（从2.0提到2.5，让验证通过的策略更受重视）
  feePenalty: -0.8, // 费率惩罚
  ...
  // 高点距离惩罚（v2: 更严格防止追高）
  peakPenalty: -6.0 // 距离历史高点<5%时扣分（从-4.0加到-6.0）
};
...
function scoreFund(
  ...
  if (fund.status !== "active" || fund.dailyLimit <= 0) {
    return {
      score: WEIGHTS.suspended,
      reason: fund.status === "suspended" ? "\u6682\u505c\u7533\u8d2d" : "\u9650\u989d\u4e3a0",
      indicators
    };
  }
  ...
  if (indicators.error || !indicators.dataPoints || indicators.dataPoints < 60) {
    return {
      score: -1,
      reason:
        "\u6570\u636e\u4e0d\u8db3\uff08" + (indicators.dataPoints || 0) + "\u65e5\uff09\uff0c\u6392\u9664\u63a8\u8350",
      indicators
    };
  }
  ...
  const yr =
    premiumInfo && premiumInfo.yearReturn !== null && premiumInfo.yearReturn !== undefined
      ? premiumInfo.yearReturn
      : indicators.annualizedReturn !== null
        ? indicators.annualizedReturn
        : indicators.yearReturn;
  if (yr && yr !== 0) {
  ...
  const y3 =
    premiumInfo && premiumInfo.threeYearReturn !== null && premiumInfo.threeYearReturn !== undefined
      ? premiumInfo.threeYearReturn
      : indicators.threeYearReturn;
  if (y3 && y3 !== 0) {
  ...
  if (indicators.sharpeRatio !== null) {
    ...
    if (Math.abs(sharpeScore) > 0.5) {
      if (sharpeScore > 0)
        reasons.push("\u590f\u666e" + indicators.sharpeRatio + "\u52a0" + round1(sharpeScore) + "\u5206");
  ...
  if (indicators.maxDrawdown !== null && indicators.maxDrawdown < 0) {
    ...
    score -= ddPenalty;
    if (ddPenalty > 1)
      reasons.push("\u6700\u5927\u56de\u64a4" + indicators.maxDrawdown + "%\u6263" + round1(ddPenalty) + "\u5206");
  }
  ...
  // [fix] 回撤因子（逆向：适度回撤是买入机会，追高才该惩罚）
  let drawdownScore;
  const dd = indicators.drawdown;
  if (dd > -3) {
    drawdownScore = -1.5; // 接近新高，风险大，扣分
  } else if (dd > -10) {
    // 回撤3%-10%：适度回撤，加分（买入机会）
    drawdownScore = Math.min(3.0, (Math.abs(dd) - 3) * 0.4);
  } else if (dd > -20) {
    // 回撤10%-20%：回撤较大，扣分
    drawdownScore = -1.0 - (Math.abs(dd) - 10) * 0.2;
  } else {
    // 回撤超过20%：回撤很大，扣分更多
    drawdownScore = Math.max(-5.0, -3.0 - (Math.abs(dd) - 20) * 0.15);
  }
  score += drawdownScore;
  if (drawdownScore > 0.5) reasons.push("回撤" + Math.abs(dd) + "%加" + round1(drawdownScore) + "分");
  else if (drawdownScore < -0.5) reasons.push("近高点" + round1(dd) + "%" + round1(drawdownScore) + "分");

  // MA偏离因子（取反：低于均线更值得买）
  // [fix] 上限10分 + 仍在加速下跌时减半，防止接飞刀
  let maScore = -indicators.maDeviation * WEIGHTS.maDeviation;
  if (maScore > 10) maScore = 10;
  if (indicators.maDeviation < -5 && indicators.recent5Change < -3) {
    maScore = maScore * 0.5;
  }
  score += maScore;
  ...
  // ========== 过热回调因子（v2: 进一步收紧阈值：30日>8%, 60日>15%, 90日>25%） ==========
  if (indicators.recent30Change && indicators.recent30Change > 8) {
    const overheat30 = (indicators.recent30Change - 8) * WEIGHTS.overheat30d;
    score += overheat30;
    if (overheat30 < -0.5)
      reasons.push("30日涨" + round1(indicators.recent30Change) + "%过热" + round1(overheat30) + "分");
  }
  if (indicators.recent60Change && indicators.recent60Change > 15) {
    const overheat60 = (indicators.recent60Change - 15) * WEIGHTS.overheat60d;
    score += overheat60;
    if (overheat60 < -0.5)
      reasons.push("60日涨" + round1(indicators.recent60Change) + "%过热" + round1(overheat60) + "分");
  }
  if (indicators.recent90Change && indicators.recent90Change > 25) {
    const overheat90 = (indicators.recent90Change - 25) * WEIGHTS.overheat90d;
    score += overheat90;
    if (overheat90 < -0.5)
      reasons.push("90日涨" + round1(indicators.recent90Change) + "%过热" + round1(overheat90) + "分");
  }

  // ========== 高点距离惩罚（v2: 扩展范围，更严格防止追高） ==========
  if (indicators.peakDistance !== null && indicators.peakDistance !== undefined) {
    if (indicators.peakDistance > -8) {
      // 距离历史高点不到8%，扣分（从5%扩展到8%）
      score += WEIGHTS.peakPenalty;
      reasons.push("近高点" + round1(indicators.peakDistance) + "%" + WEIGHTS.peakPenalty + "分");
    } else if (indicators.peakDistance > -15) {
      // 距离历史高点不到15%，减半扣分（从10%扩展到15%）
  ...
  score += change5Score;
  if (change5Score < -1)
    reasons.push("5\u65e5\u6da8" + indicators.recent5Change + "%\u6263" + round1(Math.abs(change5Score)) + "\u5206");

  // 波动率
  const volScore = indicators.volatility * WEIGHTS.volatility;
  score += volScore;
  if (volScore < -0.5)
    reasons.push("\u6ce2\u52a8" + indicators.volatility + "%\u6263" + round1(Math.abs(volScore)) + "\u5206");
  ...
  if (historyContext && historyContext.successRate !== undefined && historyContext.appearances >= 3) {
    ...
    if (histBonus > 0.3)
      reasons.push(
        "\u5386\u53f2\u6210\u529f\u7387" +
          round1(historyContext.successRate * 100) +
          "%\u52a0" +
          round1(histBonus) +
          "\u5206"
      );
    else if (histBonus < -0.3)
      reasons.push(
        "\u5386\u53f2\u63a8\u8350\u4e8f\u94b1" +
          round1((1 - historyContext.successRate) * 100) +
          "%\u6263" +
          round1(Math.abs(histBonus)) +
          "\u5206"
      );
  }
  ...
  if (fund.feeRate && fund.feeRate > 0) {
    ...
    score += feeScore;
    if (Math.abs(feeScore) > 0.3)
      reasons.push("\u8d39\u7387" + fund.feeRate + "%\u6263" + round1(Math.abs(feeScore)) + "\u5206");
  }
  ...
  if (recentPicks && recentPicks[fund.code]) {
    ...
    if (consecutiveDays >= 1)
      reasons.push(
        "\u8fde\u7eed\u63a8\u8350" + consecutiveDays + "\u5929\u6263" + round1(Math.abs(rotPenalty)) + "\u5206"
      );
  }
  ...
  if (externalSignal.score !== 0) {
    ...
    reasons.push(
      "外部信号 " +
        (externalSignal.score > 0 ? "+" : "") +
        externalSignal.score +
        "分" +
        (externalSignal.matches && externalSignal.matches.length > 0
          ? "(" + externalSignal.matches.join("/") + ")"
          : "")
    );
  }
  ...
  return {
    score: round2(score),
    reason: reasons.length > 0 ? reasons.join("\uff0c") : "\u6b63\u5e38\u8bc4\u5206",
    indicators,
...
const CORRELATION_GROUPS = {
  美股科技: "美股",
  美股宽基: "美股", // 纳指≈标普，底层都是美股大盘科技
  全球: "美股", // "全球精选/科技/成长"实际重仓美股科技
  主题: "美股" // 新能源车等主题也偏美股
};
const MAX_PER_CATEGORY = 3;
const MAX_PER_CORRELATION_GROUP = 3; // 相关性分组内最多3只（从4收紧到3）
