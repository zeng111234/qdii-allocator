...
const WEIGHTS = {
  base: 10,
  // 长期因子（核心，但权重平衡）
  yearReturn: 0.1,          // 1年收益率（降低权重，避免追涨历史赢家）
  ...
  // 过热回调因子
  overheat30d: -0.4,        // 30日涨超15%过热扣分
  overheat60d: -0.3,        // 60日涨超25%过热扣分
  overheat90d: -0.2,        // 90日涨超35%过热扣分
...
function scoreFund(fund, indicators, historyContext, recentPicks, premiumInfo, externalSignals, externalSignalMaxScore, options) {
  ...
  if (indicators.recent60Change && indicators.recent60Change > 25) {
    const overheat60 = (indicators.recent60Change - 25) * WEIGHTS.overheat60d;
    score += overheat60;
    if (overheat60 < -0.5) reasons.push("60日涨" + round1(indicators.recent60Change) + "%过热" + round1(overheat60) + "分");
  }
  if (indicators.recent90Change && indicators.recent90Change > 35) {
    const overheat90 = (indicators.recent90Change - 35) * WEIGHTS.overheat90d;
    score += overheat90;
  ...
  // [fix] 移除稀缺加分：每天只投50元，限购不影响用户决策
  // 限购只影响"能买多少"，不应该影响"该不该买"
