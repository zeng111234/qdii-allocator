...
function recordBuy(code, name, amount, nav, buyDate, settleDays) {
  ...
  try {
    ...
    investmentDiary.recordBuyDiary(buyInfo).catch(function(err) {
...
function calcHoldingDetail(holding, navCache) {
  ...
  // 计算当前市值
  const currentValue = totalShares > 0 && latestNav ? Math.round(totalShares * latestNav * 100) / 100 : null;
  const pnl = currentValue !== null ? Math.round((currentValue - totalAmount) * 100) / 100 : null;
  const pnlRate = currentValue !== null && totalAmount > 0 ? Math.round((currentValue - totalAmount) / totalAmount * 10000) / 100 : null;
