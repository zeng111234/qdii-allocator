...
/**
 * 计算结算日期（确认日）
 * QDII基金通常是T+2，国内基金T+1
 * @param {string} buyDate - 买入日期 YYYY-MM-DD
 * @param {number} settleDays - 结算天数（默认2）
 * @returns {string} 结算日期 YYYY-MM-DD
 */
...
function recordBuy(code, name, amount, nav, buyDate, settleDays) {
  ...
  if (buyDate) {
    ...
    settleDate = calcSettleDate(date, sd);
    // 结算日不能超过今天
    if (settleDate > today) {
      settleDate = today;
    }
  ...
  if (!nav && buyDate && settleDate) {
    ...
    if (navs && navs.length > 0) {
      ...
      if (cachedNav) {
        nav = cachedNav.nav;
        console.log("[持仓] 使用结算日 " + settleDate + " 净值: " + nav);
      } else {
        // 结算日没有净值，找之后最近的交易日
        ...
        if (nextNav) {
          ...
          console.log("[持仓] 结算日 " + settleDate + " 无净值，使用下一个交易日 " + nextNav.date + " 净值: " + nav);
        } else {
  ...
  console.log("[持仓] 已记录: " + (name || code) + " " + date + " 买入 " + amount + "元" +
    (settleDate ? " (结算日: " + settleDate + ")" : "") +
    (nav ? " (净值" + nav + ", 份额" + shares + ")" : ""));
  return holding;
}
...
function showTodayBuyCommands(fundsList, portfolioResult) {
  ...
  lines.push("");
  lines.push("注意: QDII基金T+2结算，系统会自动用结算日净值计算份额");
  lines.push("");
