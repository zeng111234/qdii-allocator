...
async function loadPortfolioAsync() {
  ...
  if (firebase.isFirebaseAvailable()) {
    const cloudData = await firebase.loadPortfolioFromFirebase();
    if (cloudData && cloudData.holdings && cloudData.holdings.length > 0) {
      console.log("[持仓] 从 Firebase 加载: " + cloudData.holdings.length + "只基金");
      // 同步到本地文件作为备份
      try {
        fs.writeFileSync(PORTFOLIO_FILE, JSON.stringify(cloudData, null, 2), "utf-8");
...
function savePortfolio(portfolio) {
  ...
  // 异步写入 Firebase（不阻塞主流程）
  if (firebase.isFirebaseAvailable()) {
    firebase.savePortfolioToFirebase(portfolio).then(function(ok) {
      if (ok) console.log("[持仓] 已同步到 Firebase");
    }).catch(function(err) {
      console.warn("[持仓] Firebase 同步失败:", err.message);
    });
  }
}
