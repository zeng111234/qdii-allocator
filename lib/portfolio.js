...
const PORTFOLIO_FILE = process.env.PORTFOLIO_FILE
  ? path.resolve(process.env.PORTFOLIO_FILE)
  : path.join(__dirname, "..", "data", "portfolio.json");
const FUNDS_FILE = path.join(__dirname, "..", "data", "funds.json");

function loadFundMetadataByCode() {
  try {
    const data = JSON.parse(fs.readFileSync(FUNDS_FILE, "utf8"));
    return (data.funds || []).reduce(function(result, fund) {
      result[fund.code] = fund;
      return result;
    }, {});
  } catch (e) {
    console.warn("[持仓] 基金分类加载失败:", e.message);
    return {};
  }
}
...
/**
 * 同步加载持仓（从本地文件，用于 CLI 和非关键路径）
 */
function loadPortfolio() {
  try {
    if (!fs.existsSync(PORTFOLIO_FILE)) return { holdings: [], startDate: null };
    const stat = fs.statSync(PORTFOLIO_FILE);
    // Do not trust mtime as a cache key: rapid writes on Windows can share the
    // same timestamp and would otherwise return stale holdings during imports.
    const raw = JSON.parse(fs.readFileSync(PORTFOLIO_FILE, "utf-8"));
    _portfolioInstance = Number(raw.schemaVersion) === portfolioLedger.SCHEMA_VERSION
      ? portfolioLedger.derivePortfolio(raw)
      : raw;
...
/**
 * 异步加载持仓（优先从 Firebase，回退到本地文件）
 * 用于 GitHub Actions 每日任务开始时
 */
async function loadPortfolioAsync() {
  // 尝试从 Firebase 加载
  if (firebase.isFirebaseAvailable()) {
    const cloudLedger = await firebase.loadPortfolioLedgerFromFirebase();
    if (cloudLedger && portfolioLedger.validateLedger(cloudLedger).valid) {
      const cloudData = portfolioLedger.derivePortfolio(cloudLedger);
      console.log("[持仓] 从 Firebase 私有账本加载 revision=" + cloudLedger.revision + ": " + cloudData.holdings.length + "只基金");
      // 同步到本地文件作为备份
      try {
        fs.writeFileSync(PORTFOLIO_FILE, JSON.stringify(cloudLedger, null, 2), "utf-8");
        _portfolioInstance = cloudData;
...
/**
 * 保存持仓（双写：Firebase + 本地文件）
 */
function savePortfolio(portfolio) {
  if (process.env.PORTFOLIO_READ_ONLY === "1") throw new Error("PORTFOLIO_READ_ONLY");
  // 写入本地文件
  try {
    fs.writeFileSync(PORTFOLIO_FILE, JSON.stringify(portfolio, null, 2), "utf-8");
    _portfolioInstance = portfolio;
    _portfolioMtime = fs.statSync(PORTFOLIO_FILE).mtimeMs;
  } catch(e) { console.error("[持仓] 本地保存失败:", e.message); }

  // 云端只接受 PortfolioLedgerV2；旧 holdings 模型不得再双写覆盖。
}
...
/**
 * 场外 QDII 通常按有效申请日（T 日）净值成交，T+2 是确认日而非成交净值日。
 * 节假日申请顺延时使用申请日之后第一个可用净值。
 */
function resolveTradeNav(navs, tradeDate) {
  return findNavOnOrAfter(navs, tradeDate, 7);
}
...
function shanghaiDateString(value) {
  const parts = new globalThis.Intl.DateTimeFormat("en-CA", {
    ...
    month: "2-digit",
    day: "2-digit"
  }).formatToParts(value === undefined ? new Date() : new Date(value));
  const byType = {};
...
function valuationTradingDayLag(latestDate, asOf) {
  if (!isStrictIsoDate(latestDate) || !isStrictIsoDate(asOf) || latestDate > asOf) return Infinity;
  ...
  while (cursor < end) {
    ...
    if (tradingCal.isTradingDay(date)) lag++;
  }
  return lag;
}

function valuationOptions(options) {
  const source = options || {};
  const configuredLag = Number(source.maxFreshnessLag);
  return {
    asOf: isStrictIsoDate(source.asOf) ? source.asOf : shanghaiDateString(),
    maxFreshnessLag: Number.isFinite(configuredLag) && configuredLag >= 0
      ? configuredLag
      : allocationPolicy.createAllocationPolicy().maxFreshnessLag
  };
}

function assessValuationNav(row, options) {
  const settings = valuationOptions(options);
  if (!row) return { usable: false, reason: "NAV_MISSING", nav: null, date: null, tradingDayLag: null };
  const rawNav = row.nav;
  const date = row.date;
  if (typeof rawNav !== "number" || !Number.isFinite(rawNav) || rawNav <= 0 || !isStrictIsoDate(date)) {
    return { usable: false, reason: "NAV_INVALID", nav: null, date: isStrictIsoDate(date) ? date : null, tradingDayLag: null };
  }
  if (date > settings.asOf) {
    return { usable: false, reason: "NAV_FUTURE", nav: rawNav, date: date, tradingDayLag: null };
  }
  const lag = valuationTradingDayLag(date, settings.asOf);
  if (!Number.isFinite(lag)) {
    return { usable: false, reason: "NAV_INVALID", nav: rawNav, date: date, tradingDayLag: null };
  }
  if (lag > settings.maxFreshnessLag) {
    return { usable: false, reason: "NAV_STALE", nav: rawNav, date: date, tradingDayLag: lag };
  }
  return { usable: true, reason: null, nav: rawNav, date: date, tradingDayLag: lag };
}
...
function recordBuy(code, name, amount, nav, buyDate, settleDays) {
  ...
  // 确定买入净值：按有效申请日（T 日）净值；结算日只记录确认时间。
  if (!nav && buyDate && settleDate) {
    const navs = cache[code];
    if (navs && navs.length > 0) {
      const tradeNav = resolveTradeNav(navs, date);
      if (tradeNav) {
        nav = tradeNav.nav;
        console.log("[持仓] 使用有效申请日 " + tradeNav.date + " 的净值: " + nav + "；" + settleDate + " 仅为确认日");
      } else console.warn("[持仓] 有效申请日 " + date + " 附近无净值数据，请稍后回填");
    }
  } else if (!nav && !buyDate) {
    // 没指定日期也没传净值，用最新净值
    const navs = cache[code];
 ...
 * 计算单只基金的持仓详情
 */
function calcHoldingDetail(holding, navCache, options) {
  let totalAmount = 0;
  ...
  let valuationIssue = null;
  if (navs && navs.length > 0) {
    const last = navs[navs.length - 1];
    const assessment = assessValuationNav(last, options);
    latestNav = assessment.nav;
    latestDate = assessment.date;
    navTradingDayLag = assessment.tradingDayLag;
    valuationIssue = assessment.reason;
  } else if (totalShares > 0) {
    valuationIssue = "NAV_MISSING";
  }

  const currentValue = totalShares > 0 && !valuationIssue && latestNav
    ? Math.round(totalShares * latestNav * 100) / 100
    : null;
  const pnl = currentValue !== null ? Math.round((currentValue - confirmedAmount) * 100) / 100 : null;
  const pnlRate = currentValue !== null && confirmedAmount > 0
    ? Math.round((currentValue - confirmedAmount) / confirmedAmount * 10000) / 100
    : null;

  // 计算平均成本
  const avgCost = totalShares > 0 ? Math.round(confirmedAmount / totalShares * 10000) / 10000 : null;

  // 已实现盈亏（从卖出记录）
  let totalRealizedPnl = Number(holding.realizedPnl) || 0;
  ...
  return {
    ...
    name: holding.name,
    type: holding.type,
    indexGroup: holding.indexGroup,
    riskBucket: holding.riskBucket || allocationPolicy.bucketForIndexGroup(holding.indexGroup),
    ...
    navTradingDayLag: navTradingDayLag,
    valuationIssue: totalShares > 0 ? valuationIssue : null,
    valuationComplete: totalShares <= 0 || !valuationIssue,
    ...
    buyDetails: holding.buys.map(function(b) {
      const type = String(b.type || ((Number(b.amount) < 0 || Number(b.shares) < 0) ? "SELL" : "BUY")).toUpperCase();
      return {
        type: type,
        date: b.date,
 ...
 * 计算整个组合的汇总指标
 */
function calcPortfolioSummary(options) {
  const portfolio = loadPortfolio();
  const navCache = loadNavCache();
  const fundMetadata = loadFundMetadataByCode();

  const closedPositions = Array.isArray(portfolio.closedPositions) ? portfolio.closedPositions : [];
  const closedRealizedPnl = Number(portfolio.closedRealizedPnl) || 0;
  if ((!portfolio.holdings || portfolio.holdings.length === 0) &&
      !(Number(portfolio.pendingInvested) > 0) && closedPositions.length === 0) {
    return { empty: true, holdings: [], summary: null };
  }

  const details = [];
  let totalInvested = 0;
  let pendingInvested = Number(portfolio.pendingInvested) || 0;
  let totalValue = 0;
  let hasValue = false;
  const missingValuationCodes = [];
  const valuationIssues = [];

  let totalRealizedPnl = closedRealizedPnl;
  for (let i = 0; i < portfolio.holdings.length; i++) {
    const holding = portfolio.holdings[i];
    const enrichedHolding = Object.assign({}, fundMetadata[holding.code] || {}, holding);
    const detail = calcHoldingDetail(enrichedHolding, navCache, options);
    details.push(detail);
    totalInvested += detail.confirmedAmount;
    if (!portfolio.pendingHoldings) pendingInvested += detail.pendingAmount;
    totalRealizedPnl += detail.realizedPnl || 0;
    if (detail.currentValue !== null) {
      totalValue += detail.currentValue;
      hasValue = true;
    } else if (detail.totalShares > 0) {
      missingValuationCodes.push(detail.code);
      valuationIssues.push({
        code: detail.code,
        reason: detail.valuationIssue || "NAV_MISSING",
        latestDate: detail.latestDate,
        tradingDayLag: detail.navTradingDayLag
      });
    }
  }
  totalRealizedPnl = Math.round(totalRealizedPnl * 100) / 100;
  ...
  let summary = null;
  if (hasValue || pendingInvested > 0 || totalInvested > 0 || closedPositions.length > 0) {
    const valuationComplete = missingValuationCodes.length === 0 && (totalInvested === 0 || hasValue);
    const totalPnl = valuationComplete
      ? Math.round((totalValue - totalInvested + closedRealizedPnl) * 100) / 100
      : null;
    const totalPnlRate = valuationComplete && totalInvested > 0 ? Math.round(totalPnl / totalInvested * 10000) / 100 : null;
    ...
    summary = {
      ...
      grossInvested: totalInvested + pendingInvested,
      pendingInvested: pendingInvested,
      totalValue: valuationComplete ? Math.round(totalValue * 100) / 100 : null,
      totalPnl: totalPnl,
      totalPnlRate: totalPnlRate,
      valuationComplete: valuationComplete,
      missingValuationCodes: missingValuationCodes,
      valuationIssues: valuationIssues,
...
function formatPortfolioReport(calcResult) {
  ...
  for (let i = 0; i < holdings.length; i++) {
    const h = holdings[i];
    const pnlStr = h.pnl !== null ? (h.pnl >= 0 ? "+" : "") + h.pnl + "元 (" + (h.pnlRate >= 0 ? "+" : "") + h.pnlRate + "%)" : "估值不完整(" + (h.valuationIssue || "NAV_MISSING") + ")";
    ...
    for (let j = 0; j < h.buyCount; j++) {
      ...
      let detailStr = "     " + (j+1) + ". " + b.date + " " + (isSell ? "卖出" : "买入") + Math.abs(b.amount) + "元";
  ...
  if (calcResult.summary) {
    ...
    lines.push("--- 组合汇总 ---");
    if (s.valuationComplete === false) {
      lines.push("已确认投入: " + s.totalInvested + "元 | 当前市值: 估值不完整");
      const issueText = (s.valuationIssues || []).map(function(issue) {
        return issue.code + ":" + issue.reason + (issue.latestDate ? "@" + issue.latestDate : "");
      }).join(", ");
      lines.push("净值问题: " + (issueText || (s.missingValuationCodes || []).join(", ")));
    } else {
      const totalPnlStr = (s.totalPnl >= 0 ? "+" : "") + s.totalPnl + "元 (" +
        (s.totalPnlRate >= 0 ? "+" : "") + s.totalPnlRate + "%)";
      lines.push("已确认投入: " + s.totalInvested + "元 | 总市值: " + s.totalValue + "元");
      lines.push("总盈亏: " + totalPnlStr);
    }
    if (s.pendingInvested > 0) lines.push("待确认: " + s.pendingInvested + "元（未计入市值和盈亏）");
...
function showTodayBuyCommands(fundsList, portfolioResult) {
  ...
  lines.push("");
  lines.push("注意: 15:00前的有效申请通常按T日净值成交；QDII常在T+2确认。确认延后不等于使用T+2净值。");
  lines.push("");
...
function autoSettlePending() {
  ...
  portfolioData.holdings.forEach(function(h) {
    h.buys.forEach(function(buy) {
      ...
      const settleDate = settleInfo ? settleInfo.date : null;

      const navs = cache[h.code];
      if (!navs || navs.length === 0) { skipped++; return; }

      const tradeNav = resolveTradeNav(navs, buy.date);
      const nav = tradeNav ? tradeNav.nav : null;
