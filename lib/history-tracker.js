...
function saveRecommendationPlan(plan) {
  try {
    ...
    const previous = data.records.find(function (record) { return record.date === plan.asOf; });
    const priorReturns = {};
    ...
    data.records = data.records.filter(function (record) { return record.date !== plan.asOf; });
    data.records.push(record);
