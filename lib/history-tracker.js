...
const { normalizeDate, round2, addDaysToDate, daysBetween } = require("./utils");
const fs = require("fs");
      ...
      for (let j = 0; j < allocs.length; j++) {
        ...
        if (alloc.followUp5dReturn === null) {
          ...
          for (let r = 0; r < navs.length; r++) {
            if (navs[r].date === recDate) { recIdx = r; break; }
          }
          let nav5 = null;
          ...
          if (!nav5) {
            for (let k = 0; k < navs.length; k++) {
              ...
              if (diff >= 4 && diff <= 6) { nav5 = navs[k]; break; }
            }
          }
          if (nav5) {
        ...
        if (alloc.followUp10dReturn === null) {
          ...
          for (let r2 = 0; r2 < navs.length; r2++) {
            if (navs[r2].date === recDate) { recIdx10 = r2; break; }
          }
          let nav10 = null;
          ...
          if (!nav10) {
            for (let k2 = 0; k2 < navs.length; k2++) {
              ...
              if (diff2 >= 9 && diff2 <= 12) { nav10 = navs[k2]; break; }
            }
          }
          if (nav10) {
