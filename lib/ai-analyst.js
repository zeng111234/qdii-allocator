...
function buildDecisionPrompt(result) {
  ...
  if (result.externalSignals && (result.externalSignals.status === "ok" || result.externalSignals.status === "cached")) {
    ...
    if (result.externalSignals.tickerOpinions && result.externalSignals.tickerOpinions.length > 0) {
      ...
      var topTickers = result.externalSignals.tickerOpinions.slice(0, 10);
      for (var tk = 0; tk < topTickers.length; tk++) {
        var tko = topTickers[tk];
        var sentimentLabel = tko.sentiment === "bullish" ? "\ud83d\udfe2\u770b\u597d" : (tko.sentiment === "bearish" ? "\ud83d\udd34\u770b\u6de1" : "\ud83d\udfe1\u4e2d\u6027");
    ...
    if (result.externalSignals.opinionSummaries && result.externalSignals.opinionSummaries.length > 0) {
      lines.push("\u2022 \u6700\u65b0\u63a8\u6587\u6458\u8981\uff1a");
      for (var os = 0; os < result.externalSignals.opinionSummaries.length; os++) {
        var opinion = result.externalSignals.opinionSummaries[os];
        var tickersStr = opinion.tickers ? " [" + opinion.tickers + "]" : "";
  ...
  if (result.externalSignals && result.externalSignals.newDirections && result.externalSignals.newDirections.newThemes.length > 0) {
    var dirs = result.externalSignals.newDirections;
    var uncovered = dirs.newThemes.filter(function(t) { return !t.covered; });
    if (uncovered.length > 0) {
      lines.push("--- \ud83d\udd0d \u5927\u795e\u63d0\u5230\u7684\u65b0\u6295\u8d44\u65b9\u5411\uff08\u5f53\u524d\u57fa\u91d1\u6c60\u672a\u8986\u76d6\uff09 ---");
      for (var nd = 0; nd < uncovered.length; nd++) {
        var dir = uncovered[nd];
        var dirSentiment = dir.sentiment === "bullish" ? "\ud83d\udfe2\u770b\u597d" : (dir.sentiment === "bearish" ? "\ud83d\udd34\u770b\u6de1" : "\ud83d\udfe1\u4e2d\u6027");
  ...
  if (result.portfolio && !result.portfolio.empty && result.portfolio.summary) {
    ...
    if (result.portfolio.holdings && result.portfolio.holdings.length > 0) {
      for (var pi = 0; pi < result.portfolio.holdings.length; pi++) {
        var ph = result.portfolio.holdings[pi];
        var phPnl = ph.pnl !== null ? ((ph.pnl >= 0 ? "+" : "") + ph.pnl + "\u5143(" + (ph.pnlRate >= 0 ? "+" : "") + ph.pnlRate + "%)") : "\u5f85\u66f4\u65b0";
  ...
  if (result.alternatives && result.alternatives.length > 0) {
    lines.push("--- \u66ff\u4ee3\u65b9\u6848 ---");
    for (var ai = 0; ai < result.alternatives.length; ai++) {
      var alt = result.alternatives[ai];
