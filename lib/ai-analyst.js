...
async function generateCommentary(result, llmConfig) {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (result.recommendationPlan) {
        const validation = recommendationEngine.validateAIOutput(result.recommendationPlan, text);
        if (!validation.valid) {
          console.warn("[AI] 输出越权或结构错误，整段丢弃: " + validation.errors.join(", "));
          return "[AI 解读已拒绝：输出与 RecommendationPlan 不一致]";
        }
        const value = validation.value;
        const lines = [value.summary || ("系统计划为 " + value.action)];
        (value.candidates || []).forEach(function (candidate) {
          if (candidate.explanation) lines.push(candidate.code + "：" + candidate.explanation);
        });
        return lines.join("\n");
      }
      // 反幻觉验证
...
function buildDecisionPrompt(result) {
  if (result.recommendationPlan) {
    ...
    const expected = {
      action: plan.action,
      candidates: (plan.candidates || []).map(function (candidate) {
        return { code: candidate.code, proposedAmount: candidate.proposedAmount, explanation: "只解释计划内理由" };
      }),
      summary: "解释计划状态"
    };
    return [
      "你只能解释下面由确定性代码生成的 RecommendationPlan。",
      "禁止新增基金、删除候选、修改 action 或 proposedAmount；PAUSE/HOLD 不得解释为可以买入。",
      "只输出 JSON，不得输出思维过程、Markdown 或额外文本。",
      "输出骨架（只能修改 explanation 和 summary 文本）: " + JSON.stringify(expected),
      "RecommendationPlan:",
      JSON.stringify(plan)
    ].join("\n");
  }
  ...
  if (result.portfolio && !result.portfolio.empty && result.portfolio.summary) {
    ...
    lines.push("--- \ud83d\udcb0 \u6211\u7684\u6301\u4ed3 ---");
    if (ps.valuationComplete === false) {
      lines.push("\u5df2\u786e\u8ba4\u6295\u5165: " + ps.totalInvested + "\u5143 | \u4f30\u503c\u4e0d\u5b8c\u6574，\u51c0\u503c\u95ee\u9898: " +
        formatValuationIssues(ps.valuationIssues, ps.missingValuationCodes) + "；\u4e0d\u5f97\u63a8\u65ad\u5e02\u503c\u6216\u76c8\u4e8f");
    } else {
      lines.push("\u5df2\u786e\u8ba4\u6295\u5165: " + ps.totalInvested + "\u5143 | \u5f53\u524d\u5e02\u503c: " + ps.totalValue + "\u5143 | \u76c8\u4e8f: " +
        (ps.totalPnl >= 0 ? "+" : "") + ps.totalPnl + "\u5143 (" + (ps.totalPnlRate >= 0 ? "+" : "") + ps.totalPnlRate + "%)");
    }
    if (Number(ps.pendingInvested) > 0) lines.push("\u5f85\u786e\u8ba4: " + ps.pendingInvested + "\u5143（\u672a\u8ba1\u5165\u5e02\u503c\u548c\u76c8\u4e8f）");
    ...
    if (result.portfolio.holdings && result.portfolio.holdings.length > 0) {
      for (let pi = 0; pi < result.portfolio.holdings.length; pi++) {
        ...
        const phPnl = ph.pnl !== null ? ((ph.pnl >= 0 ? "+" : "") + ph.pnl + "\u5143(" + (ph.pnlRate >= 0 ? "+" : "") + ph.pnlRate + "%)") :
          ("\u4f30\u503c\u4e0d\u5b8c\u6574(" + (ph.valuationIssue || "NAV_MISSING") + ")");
        lines.push("  " + ph.name + "(" + ph.code + "): \u5df2\u786e\u8ba4\u6295\u5165" +
          (ph.confirmedAmount !== undefined ? ph.confirmedAmount : ph.totalAmount) + "\u5143, \u76c8\u4e8f" + phPnl);
      }
    }
    lines.push("");
  ...
  lines.push("   \u91cd\u70b9\u53c2\u8003\u8bc4\u5206\u3001\u56de\u64a4\u3001\u590f\u666e\u3001\u4f30\u503c\u548c\u5386\u53f2\u63a8\u8350\u80dc\u7387\uff0c\u4e0d\u8981\u53ea\u770b\u8fc7\u53bb\u6536\u76ca");
  lines.push("   \u540c\u6307\u6570\u57fa\u91d1\u5e95\u5c42\u91cd\u53e0\uff08\u5982\u591a\u53ea\u7eb3\u6307\u90fd\u662f\u82f1\u4f1f\u8fbe+\u82f9\u679c\uff09\uff0c\u6ce8\u610f\u5206\u6563");
  ...
  lines.push("   \u793a\u4f8b\uff1a\u201c\u5e02\u573a\u521a\u8dcc\u4e864%\uff0c\u5176\u5b9e\u53cd\u800c\u662f\u5b9a\u6295\u7684\u597d\u65f6\u673a\u2014\u2014\u4e0b\u8dcc\u65f6\u4e70\u5165\u6210\u672c\u66f4\u4f4e\u3002\u5c0f\u989d\u5b9a\u6295\u4e0d\u7528\u592a\u62c5\u5fc3\uff0c\u575a\u6301\u5c31\u884c\u3002\u201d");
  lines.push("");
...
module.exports = { generateCommentary: generateCommentary, callLLM: callLLM, buildDecisionPrompt: buildDecisionPrompt };
