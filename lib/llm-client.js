      ...
      res.on("end", function() {
        ...
        try {
          ...
          if (json.choices && json.choices[0]) {
            ...
            // [fix] 不再把 reasoning_content(思维链) 当正文: 正文为空视为失败, 由上层重试/降级,
            // 避免早报/AI报告显示模型的思考过程
            let text = "";
            if (typeof msg.content === "string") text = msg.content.trim();
            else if (Array.isArray(msg.content)) text = msg.content.map(function (c) { return (c && c.text) || ""; }).join("").trim();
            if (text.length > 0) {
              resolve(text);
            } else {
              reject(new Error("LLM returned empty content (reasoning only, content missing)"));
            }
          } else if (json.error) {
