...
function analyzeNewDirections(tickerOpinions, existingFunds) {
  ...
  // 1. 统计大神提到的投资主题
  var themeStats = {};
  for (var i = 0; i < tickerOpinions.length; i++) {
    var tko = tickerOpinions[i];
    var mapping = TICKER_THEME_MAP[tko.ticker];
    if (!mapping) continue;
  ...
  // 2. 检查现有基金池覆盖情况
  var fundTypes = (existingFunds || []).map(function(f) {
    return ((f.type || "") + " " + (f.name || "") + " " + (f.note || "")).toLowerCase();
  }).join(" ");

  // 纳指100覆盖了大部分AI/半导体股票
  var hasNasdaq = fundTypes.indexOf("纳指") >= 0 || fundTypes.indexOf("nasdaq") >= 0;
  var hasSp500 = fundTypes.indexOf("标普") >= 0 || fundTypes.indexOf("sp500") >= 0;
  var hasHk = fundTypes.indexOf("港") >= 0 || fundTypes.indexOf("恒生") >= 0;
  var hasGlobal = fundTypes.indexOf("全球") >= 0;
  var hasAsia = fundTypes.indexOf("亚太") >= 0;

  var newThemes = [];
  var themeKeys = Object.keys(themeStats);
  for (var j = 0; j < themeKeys.length; j++) {
    var stat = themeStats[themeKeys[j]];
    // 检查关键词是否在现有基金池中出现
    var covered = false;
    for (var k = 0; k < stat.keywords.length; k++) {
    ...
    if (!covered) {
      var mapping = TICKER_THEME_MAP[stat.tickers[0].replace("$", "")];
      if (mapping && mapping.related) {
        for (var r = 0; r < mapping.related.length; r++) {
          var rel = mapping.related[r];
    ...
    var avgSentiment = stat.sentiment > 0 ? "bullish" : (stat.sentiment < 0 ? "bearish" : "neutral");
    newThemes.push({
      theme: stat.theme,
  ...
  newThemes.sort(function(a, b) {
    ...
    if (a.sentiment !== b.sentiment) {
      var sOrder = { bullish: 0, neutral: 1, bearish: 2 };
  ...
  // 3. 生成缺口摘要
  var gaps = newThemes.filter(function(t) { return !t.covered && t.sentiment === "bullish"; });
  var gapSummary = "";
  if (gaps.length > 0) {
...
function buildMirrorUrls(config) {
  ...
  var urls = mirrors.map(function(item) {
  ...
  var selfHosted = config && config.rsshubUrl ? config.rsshubUrl : "";
  if (selfHosted) {
    var base = selfHosted.replace(/\/+$/, "");
    var selfUrl = base + "/twitter/user/" + encodeURIComponent(handle);
...
function extractTickerOpinions(items) {
  var tickerMap = {};
  for (var i = 0; i < items.length; i++) {
    var text = items[i].title + " " + (items[i].text || "");
    // 提取 $TICKER 格式的股票代码
    var tickerRe = /\$([A-Z]{2,6})/g;
    var m;
    while ((m = tickerRe.exec(text)) !== null) {
      ...
      var pos = m.index;
      var snippet = text.substring(Math.max(0, pos - 40), Math.min(text.length, pos + 120))
        .replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
      if (snippet.length > 10 && tickerMap[ticker].snippets.length < 2) {
        tickerMap[ticker].snippets.push(snippet);
      }
      // 该条推文对该 ticker 的情绪
      var itemSentiment = scoreText(text);
      tickerMap[ticker].sentiment += itemSentiment;
    }
  }
  // 排序：提及次数多的在前
  var result = Object.values(tickerMap).map(function(t) {
...
async function fetchExternalSignals(config) {
  ...
  for (var i = 0; i < rssUrls.length; i++) {
    ...
    try {
      ...
      var tickerOpinions = extractTickerOpinions(items);
      var opinionSummaries = extractOpinionSummaries(items, 5);
