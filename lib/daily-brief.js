...
function buildPrompt(result, portfolioData) {
  ...
  if (result && result.recommendationPlan) {
    ...
    lines.push("不得新增基金、修改金额，或把 PAUSE/HOLD 描述成可以买入。");
  ...
  if (portfolioData && portfolioData.holdings && portfolioData.holdings.length > 0) {
    ...
    for (let j = 0; j < portfolioData.holdings.length; j++) {
      ...
      const shares = h.buys.reduce(function (s, b) {
        return s + (b.shares || 0);
      }, 0);
  ...
  if (result && result.marketTemperature) {
    ...
    parts.push(
      "🌡️ 市场温度 " +
        ...
        mt.level +
        ")，建议投入" +
        (result.budgetInfo ? result.budgetInfo.adjustedBudget || result.budget : 50) +
        "元。"
    );
  }
