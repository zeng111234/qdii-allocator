...
function buildPrompt(question) {
  ...
  if (scenario === "buy_recommendation") {
    ...
    lines.push(
      "2. 从持仓集中度出发：先统计持仓中同类型基金数量，如果用户已持有3+只纳指/同类基金，必须优先推荐非同类、非高相关标的"
    );
    lines.push("3. 结合评分+风险：评分≥20且近期回撤/波动可控的基金优先；限购额度仅代表购买限制，不代表基金质量");
    lines.push("4. 给出具体建议：推荐2-3只，每只说明理由和建议金额（30-100元）");
    lines.push("5. 给出风险提醒：当前组合最大回撤、行业集中度、持仓盈亏情况");
    lines.push("");
  ...
  if (scenario === "buy_recommendation") {
    ...
    lines.push("2. 推荐理由（结合评分、风险指标和持仓分散度，限购仅作参考）");
    lines.push("3. 风险提示（回撤、相关性、持仓类型分布、底层重叠）");
    lines.push("4. 操作建议（具体买入金额、时机与止损/止盈提示）");
    lines.push("");
  } else if (scenario === "market_sentiment") {
    lines.push("=== 输出格式要求 ===");
    lines.push("请按以下结构回答：");
