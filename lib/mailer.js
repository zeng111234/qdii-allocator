...
function buildEmailHtml(textContent, aiCommentary, result) {
  ...
  if (result && result.fundChanges && result.fundChanges.length > 0) {
    var changesList = result.fundChanges.map(function(c) {
  ...
  if (result && result.risk) {
    var rk = result.risk;
    var healthColor = rk.healthScore >= 80 ? "#27ae60" : (rk.healthScore >= 60 ? "#f39c12" : "#e74c3c");
    riskHtml = "<div style=\"margin-top:12px;padding:12px;background:linear-gradient(135deg,#2c3e50 0%,#34495e 100%);border-radius:10px;color:#fff\">"
      ...
      + "<div style=\"font-size:18px;font-weight:bold;color:" + healthColor + "\">" + rk.healthScore + "</div></div>"
      + "<div style=\"flex:1;text-align:center;padding:6px;background:rgba(255,255,255,0.15);border-radius:6px\">"
      + "<div style=\"font-size:10px;opacity:0.8\">\u590f\u666e</div>"
      + "<div style=\"font-size:16px;font-weight:bold\">" + rk.portfolioSharpe + "</div></div>"
      + "<div style=\"flex:1;text-align:center;padding:6px;background:rgba(255,255,255,0.15);border-radius:6px\">"
      + "<div style=\"font-size:10px;opacity:0.8\">\u56de\u64a4</div>"
      + "<div style=\"font-size:16px;font-weight:bold;color:#e74c3c\">" + rk.portfolioMaxDrawdown + "%</div></div>"
      + "</div>"
      + "<div style=\"font-size:11px;opacity:0.9\">" + esc(rk.concentration.dominantType) + " \u5360\u6bd4 " + rk.concentration.dominantWeight + "%"
  ...
  if (result && result.alternatives && result.alternatives.length > 0) {
    var altItems = result.alternatives.map(function(a) {
      var altNames = a.alternatives.map(function(alt) { return esc(alt.name); }).join("\u3001");
  ...
  var buyHintsHtml = "";
  if (result && result.ranked && result.ranked.length > 0) {
    var topCodes = result.ranked.slice(0, 5).map(function(f) { return f.code; });
    var quickCmd = "node index.js --quick-add \"" + topCodes.map(function(c) { return c + " 10"; }).join(", ") + "\"";
  ...
  if (result && result.portfolio && !result.portfolio.empty && result.portfolio.summary) {
    var ps = result.portfolio.summary;
    var pnlColor = ps.totalPnl >= 0 ? "#27ae60" : "#e74c3c";
    var pnlSign = ps.totalPnl >= 0 ? "+" : "";
    portfolioHtml = "<div style=\"margin-bottom:16px;padding:12px;background:linear-gradient(135deg,#0a3d62 0%,#1e5f74 100%);border-radius:10px;color:#fff\">"
      ...
      + "<div style=\"font-size:16px;font-weight:bold\">" + ps.totalInvested + "</div></div>"
      + "<div style=\"flex:1;text-align:center;padding:6px;background:rgba(255,255,255,0.15);border-radius:6px\">"
      + "<div style=\"font-size:10px;opacity:0.8\">\u5f53\u524d\u5e02\u503c</div>"
      + "<div style=\"font-size:16px;font-weight:bold\">" + ps.totalValue + "</div></div>"
      + "<div style=\"flex:1;text-align:center;padding:6px;background:rgba(255,255,255,0.15);border-radius:6px\">"
      + "<div style=\"font-size:10px;opacity:0.8\">\u76c8\u4e8f</div>"
      + "<div style=\"font-size:16px;font-weight:bold;color:" + pnlColor + "\">" + pnlSign + ps.totalPnl + "</div></div>"
      + "</div>";
    if (result.portfolio.holdings && result.portfolio.holdings.length > 0) {
      ...
      for (var hi = 0; hi < result.portfolio.holdings.length; hi++) {
        var h = result.portfolio.holdings[hi];
        var hPnlColor = (h.pnl !== null && h.pnl >= 0) ? "#27ae60" : "#e74c3c";
        var hPnlStr = h.pnl !== null ? ((h.pnl >= 0 ? "+" : "") + h.pnlRate + "%") : "-";
        portfolioHtml += "<div>" + esc(h.name) + " " + (h.totalAmount + "\u5143") + " <span style=\"color:" + hPnlColor + "\">" + hPnlStr + "</span></div>";
      }
      portfolioHtml += "</div>";
    }
    portfolioHtml += "</div>";
