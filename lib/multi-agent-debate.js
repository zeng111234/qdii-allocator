...
function buildRiskManagerPrompt(fundData, portfolioData, marketContext) {
  ...
  if (portfolioData && !portfolioData.empty) {
    lines.push("--- 当前持仓 ---");
    lines.push("已确认投入：" + portfolioData.summary.totalInvested + "元");
    if (portfolioData.summary.valuationComplete === false) {
      lines.push("估值不完整：缺少 " + (portfolioData.summary.missingValuationCodes || []).join(",") + " 净值；不得推断市值或盈亏");
    } else {
      lines.push("当前市值：" + portfolioData.summary.totalValue + "元");
      lines.push("盈亏：" + portfolioData.summary.totalPnl + "元");
    }
    if (Number(portfolioData.summary.pendingInvested) > 0) {
      lines.push("待确认：" + portfolioData.summary.pendingInvested + "元（未计入市值和盈亏）");
    }
    ...
    if (portfolioData.holdings && portfolioData.holdings.length > 0) {
      ...
      portfolioData.holdings.forEach(function(h) {
        const amount = h.confirmedAmount !== undefined ? h.confirmedAmount : h.totalAmount;
        const pnlRate = h.pnlRate === null || h.pnlRate === undefined ? "待更新" : ((h.pnlRate >= 0 ? "+" : "") + h.pnlRate + "%");
        lines.push("  " + h.name + ": " + amount + "元 (" + pnlRate + ")");
      });
    }
    lines.push("");
  }
 ...
 * 解析 Risk Manager 的响应
 */
function parseRiskManagerResponse(response, _fundData, _portfolioData) {
  if (!response) return null;
  let riskLevel = '中';
