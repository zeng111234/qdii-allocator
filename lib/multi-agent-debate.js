...
const { callLLM } = require("./ai-analyst");
...
function extractFundAnalysis(text, fundData) {
  ...
  for (var i = 0; i < fundData.length; i++) {
    var fund = fundData[i];
    var name = fund.name || fund.code;
    // 尝试匹配基金名称或代码附近的评分
    var escapedName = name.replace(/([.*+?^${}()|[\]\\])/g, '\\$1');
    var fundPattern = new RegExp(escapedName + '|' + fund.code + '[^\\n]*', 'i');
    var fundSection = '';
    var lines = text.split('\n');
    var inSection = false;
    ...
    var score = extractScore(fundSection, [
      /(?:评分|得分|分数)[：:]\s*(\d+(?:\.\d+)?)/,
      /(\d+(?:\.\d+)?)\s*[/／]\s*10/,
      /(?:看多|看空|风险)评[分估][：:]\s*(\d+(?:\.\d+)?)/
    ]) || 5;
    
    var reasons = extractBullets(fundSection).slice(0, 3);
...
function parseRiskManagerResponse(response, fundData, portfolioData) {
  if (!response) return null;
  var riskLevel = '中';
  var levelMatch = response.match(/(?:风险等级|风险水平)[：:]\s*(低|中|高|极高)/);
