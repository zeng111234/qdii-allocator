...
async function runBacktest(funds, config) {
  ...
  // Max drawdown (geometric, empyrical-style)
  let cumValue = 1.0; // 起始值 = 1
  let peak = 1.0;
  let maxDrawdown = 0;
  for (let ri = 0; ri < valid5d.length; ri++) {
    cumValue = cumValue * (1 + valid5d[ri].avgReturn5d / 100);
    if (cumValue > peak) peak = cumValue;
    const dd = (cumValue - peak) / peak * 100;
...
async function runWeightOptimization(funds, config) {
  ...
  // ������������6���ؼ�������
  // [fix] reduced grid: 2^6=64 combos instead of 4*4*3*3*3*3=1296
  const grid = {
    yearReturn:     [0.15, 0.3],
    sharpeRatio:    [3.0, 5.0],
    maxDrawdown:    [0.1, 0.2],
