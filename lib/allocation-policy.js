...
function holdingAmount(holding, navCache) {
  ...
  if (Number(holding.totalShares) > 0 && latest && Number(latest.nav) > 0) {
    return Number(holding.totalShares) * Number(latest.nav);
  }
  if (latest && Number(latest.nav) > 0 && Array.isArray(holding.buys)) {
    return holding.buys.reduce(function (sum, buy) {
      if (Number(buy.shares) > 0) return sum + Number(buy.shares) * Number(latest.nav);
      if (Number(buy.nav) > 0) return sum + Number(buy.amount || 0) * Number(latest.nav) / Number(buy.nav);
...
function calculateBucketExposure(holdings, funds, cash, navCache) {
  const fundMap = {};
  ...
  (holdings || []).forEach(function (holding) {
    ...
    values[bucket || "UNKNOWN"] += holdingAmount(holding, navCache);
  });
