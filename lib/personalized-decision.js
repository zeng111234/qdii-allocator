(function (root, factory) {
  ...
  function buildRouteDecision(input) {
    ...
    const tacticalBuckets = ["US_BROAD", "NON_US", "DEFENSIVE"];
    ...
    bucketOrder.forEach(function (bucket) {
      ...
      diagnostics.eligibleHeldChannelCount += wrappers.length;
    ...
    if (diagnostics.requestedBudget > 0 && routes.length === 0) {
      ...
      else if (diagnostics.eligibleHeldChannelCount === 0) diagnostics.blockReasons.push("NO_ELIGIBLE_EXISTING_CHANNEL");
  ...
  function hardPausePlan(base, details, reasons) {
    ...
    return Object.assign({}, base, details, {
      ...
      routeDiagnostics: { requestedBudget: 0, allocatedBudget: 0, eligibleBuckets: [], eligibleHeldChannelCount: 0, blockReasons: Array.from(new Set(reasons)) },
      executionRoutes: [],
  ...
  function personalizePlan(input) {
    ...
    return Object.assign({}, base, details, {
      action: mode,
      decisionMode: mode === "BUY" ? "NORMAL_BUY" : "TACTICAL_DCA",
