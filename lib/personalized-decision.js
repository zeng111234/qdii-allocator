(function (root, factory) {
  ...
  function getRiskAnchorMetrics(ledger, decisionState, currentValue) {
    ...
    return {
      ...
      drawdown: round(Number(currentValue) / adjustedValue - 1, 4)
    };
  }
  ...
  function buildRouteDecision(input) {
    ...
    const bucketOrder = Object.keys(settings.targetGap).filter(function (bucket) {
      ...
      return tactical ? bucket === "US_BROAD" : Number(settings.targetGap[bucket]) > 0;
    ...
    bucketOrder.forEach(function (bucket) {
      ...
      const wrapperSource = tactical
        ? (settings.funds || [])
        : Array.from(heldCodes).map(function (code) { return fundMap[code]; });
      ...
      wrappers.forEach(function (fund) {
        ...
        const bucketCapacity = tactical
          ? Infinity
          : Math.max(0, Number(settings.targetGap[bucket]) * settings.metrics.totalValue);
        ...
        routes.push({
          ...
          reason: tactical ? "标普500基准定投：主动策略未证明超额时回到低费率标普通道" : "通过验收：按目标缺口和费率路由"
        });
  ...
  function hardPausePlan(base, details, reasons) {
    ...
    const hasRiskStop = reasons.indexOf("RISK_ANCHOR_DRAWDOWN_10") >= 0;
  ...
  function personalizePlan(input) {
    ...
    const anchorMetrics = getRiskAnchorMetrics(ledger, decisionState, metrics.investedValue);
    ...
    const details = {
      ...
      strategyVersion: base.strategyVersion || "allocation-v2.4-monthly-alpha-gate",
    ...
    const dailyLimit = mode === "BUY" ? policy.maxDailyBudget : (tacticalOverheat ? policy.tacticalHotDailyBudget :
    ...
    if (requestedBudget <= 0) routeDiagnostics.blockReasons = ["WEEKLY_BUDGET_EXHAUSTED"];
    ...
    return Object.assign({}, base, details, {
      action: mode,
      decisionMode: mode === "BUY" ? "NORMAL_BUY" : "BASELINE_DCA",
      recommendationBasis: mode === "BUY" ? "LIVE_ACCEPTANCE_AND_TARGET_ALLOCATION" : "SP500_BASELINE_FALLBACK",
      expectedEdge: mode === "BUY" ? "ACCEPTANCE_PASSED" : "MATCH_BASELINE_NOT_ALPHA",
      blockedStage: budget > 0 ? null : (requestedBudget <= 0 ? "BUDGET_CAP" : "ROUTE_BLOCKED"),
      pauseReasons: mode === "TACTICAL_PAUSE" ? Array.from(new Set(tacticalReasons.concat("SP500_BASELINE_ONLY"))) : [],
      budget: budget,
