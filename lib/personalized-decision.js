(function (root, factory) {
  ...
  function buildRoutes(input) {
    ...
    const allowedBuckets = tactical ? new Set(["US_BROAD", "NON_US", "DEFENSIVE"]) : new Set(["US_BROAD", "GROWTH_TECH", "NON_US", "DEFENSIVE"]);
  ...
  function personalizePlan(input) {
    ...
    const dailyLimit = mode === "BUY" ? policy.maxDailyBudget : policy.tacticalDailyBudget;
    ...
    const routes = buildRoutes({
      ...
      metrics: metrics,
      targetGap: gaps,
      growthBlocked: anchorDrawdown !== null && anchorDrawdown <= policy.growthStopDrawdown
    });
