...
// [修复] 原问题：loadNavCache() 每次调用都全量读取+JSON.parse，主流程调用7+次
let _navCacheInstance = null;
let _navCacheMtime = 0;

/**
 * 加载净值缓存（带单例缓存，文件未变化时直接返回内存对象）
 * @returns {Object} { fundCode: [{ date, nav, accNav, changeRate }] }
 */
function loadNavCache() {
  try {
    if (!fs.existsSync(NAV_CACHE_FILE)) return {};
    const stat = fs.statSync(NAV_CACHE_FILE);
    if (_navCacheInstance && stat.mtimeMs === _navCacheMtime) {
      return _navCacheInstance;
    }
    _navCacheInstance = JSON.parse(fs.readFileSync(NAV_CACHE_FILE, "utf-8"));
    _navCacheMtime = stat.mtimeMs;
    return _navCacheInstance;
  } catch (e) {}
  return {};
}
 ...
 */
// [修复] 原问题：不检查HTTP状态码，4xx/5xx也会resolve导致后续JSON.parse失败
...
async function httpGetWithRetry(url, opts) {
  ...
  const requestedAttempts = opts && Number(opts.maxRetries);
  const maxRetries = Math.min(2, Math.max(1, requestedAttempts || 2));
  const retryDelays = (opts && opts.retryDelays) || [1000];
