...
const { loadNavCache, saveNavCache, httpGet, httpGetWithRetry, round2 } = require("./utils");
const db = require("./db");
...
function hasMaterialFundIdentityMismatch(fund, officialInfo) {
  if (!fund || !officialInfo) return false;
  const catalogCode = String(fund.code || "").trim();
  const officialCode = String(officialInfo.code || catalogCode).trim();
  if (catalogCode && officialCode && catalogCode !== officialCode) return true;
  const catalogName = String(fund.name || "").trim();
  const officialName = String(officialInfo.name || "").trim();
  const company = String(officialInfo.company || "").trim();
  if (!catalogName || (!officialName && !company)) return false;
  if (officialName && (catalogName.includes(officialName) || officialName.includes(catalogName))) return false;
  const catalogPrefix = catalogName.slice(0, 2);
  const officialPrefix = officialName.slice(0, 2);
  if (catalogPrefix && officialPrefix && catalogPrefix !== officialPrefix) return true;
  const companyPrefix = company.slice(0, 2);
  if (!officialName && companyPrefix && catalogName.length >= 2 && !catalogName.startsWith(companyPrefix)) return true;
  const officialThemePatterns = {
    SPX500: /标普\s*500/i,
    SPX500_EQUAL_WEIGHT: /标普\s*500.*等权|等权.*标普\s*500/i,
    NDX100: /纳斯达克\s*100|纳指\s*100/i,
    OIL: /石油|油气/i,
    GLOBAL_REIT: /不动产|房地产|REIT/i,
    CN_DIVIDEND: /红利/i,
    CN_SHORT_BOND: /债券|短债/i
  };
  const themePattern = officialThemePatterns[fund.indexGroup];
  if (themePattern && officialName && !themePattern.test(officialName)) return true;
  return false;
}
...
async function getFundNavHistory(fundCode, days) {
  ...
  for (let page = 1; page <= pagesToFetch; page++) {
    ...
    try {
      const raw = await httpGetWithRetry(url, { timeoutMs: 20000, maxRetries: 2, retryDelays: [2000] });
...
function calcIndicators(navHistory) {
  ...
  // peak distance for anti-chase-high
  const allTimeHigh = Math.max.apply(null, navs);
  const peakDistance = r2(((latest - allTimeHigh) / allTimeHigh) * 100);
...
async function getFundBasicInfo(fundCode) {
  ...
  try {
    ...
    const result = {
      code: String(data.FCODE || fundCode),
      name: String(data.SHORTNAME || ""),
      company: String(data.JJGS || ""),
      category: String(data.FTYPE || ""),
...
async function getMarketSnapshot() {
  ...
  // [fix] 加 f9(PE-TTM) 字段用于估值评分
  const url = "https://push2.eastmoney.com/api/qt/ulist.np/get?fields=f2,f3,f4,f9,f12,f14&secids=" + secids;
  // 首次请求失败后最多重试一次。
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (json.data && json.data.diff) {
        ...
        for (let i = 0; i < json.data.diff.length; i++) {
          ...
          result.push({
            ...
            change: d.f3 / 100,
            changeAmt: d.f4 / 100,
            pe: d.f9 && d.f9 !== "-" ? d.f9 / 100 : null  // PE-TTM
          });
        }
        return result;
      }
      ...
      if (attempt < 1) await new Promise(function(r) { setTimeout(r, 1000); });
      else console.warn("[market] snapshot error after 2 attempts:", e.message);
    }
  }
  return [];
}
...
async function getValuationData() {
  ...
  const valuation = { vix: null, indices: {}, overall: "neutral" };

  for (let i = 0; i < snapshot.length; i++) {
    const item = snapshot[i];
    if (item.code === "VIXF") {
      valuation.vix = item.price;
    }
    if (item.pe !== null && item.pe !== undefined) {
      valuation.indices[item.code] = { name: item.name, pe: item.pe };
    }
  }

  // VIX-based market temperature
  if (valuation.vix !== null) {
    if (valuation.vix >= 30) valuation.overall = "fear";
    else if (valuation.vix >= 20) valuation.overall = "cautious";
    else if (valuation.vix <= 12) valuation.overall = "greedy";
    else valuation.overall = "neutral";
  }
 ...
 * 基于VIX和近期涨跌判断市场冷热
 *
 * @returns {Object} { temperature: 0-100, level, multiplier, reason, vix, dailyChange }
 *   multiplier: 建议定投金额倍数 (0.6-1.3)
 */
async function getMarketTemperature() {
  ...
  for (let i = 0; i < snapshot.length; i++) {
    const item = snapshot[i];
    if (item.code === "VIXF") vix = item.price;
    if (item.code === "NDX") ndxChange = item.change;
    if (item.code === "SPX") spxChange = item.change;
  }
  ...
  if (avgChange > 2) { temp += 10; reasons.push("今日大涨" + round2(avgChange) + "%"); }
  else if (avgChange > 1) { temp += 5; }
  else if (avgChange < -2) { temp -= 10; reasons.push("今日大跌" + round2(avgChange) + "%"); }
  else if (avgChange < -1) { temp -= 5; }
...
module.exports = { getFundNavHistory: getFundNavHistory, calcIndicators: calcIndicators, getFundPurchaseInfo: getFundPurchaseInfo, getPremiumRate: getPremiumRate, getFundBasicInfo: getFundBasicInfo, hasMaterialFundIdentityMismatch: hasMaterialFundIdentityMismatch, getMarketSnapshot: getMarketSnapshot, getMarketNews: getMarketNews, getMarketSentiment: getMarketSentiment, getGlobalNews: getGlobalNews, getUSMarketNews: getUSMarketNews, getNewsSentiment: getNewsSentiment, getXueqiuHot: getXueqiuHot, getFundAnnouncements: getFundAnnouncements, loadNavCache: loadNavCache, cleanStaleCache: cleanStaleCache, initNavDb: initNavDb, getValuationData: getValuationData, getMarketTemperature: getMarketTemperature };
