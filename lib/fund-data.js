...
async function getFundNavHistory(fundCode, days) {
  ...
  for (var page = 1; page <= pagesToFetch; page++) {
    ...
    if (lastErr && attempt >= 3) {
      console.error("[data] fund " + fundCode + " page " + page + " failed after 3 retries");
      fetchSuccess = false;
    }
  ...
  // 如果API完全失败但有缓存，使用缓存（绝不丢失已有数据）
  if (allRecords.length === 0 && cached.length > 0) {
    console.warn("[data] fund " + fundCode + ": API failed, using " + cached.length + " cached records");
...
async function getFundBasicInfo(fundCode) {
  ...
  var maxRetries = 3;
  var delays = [1000, 2000, 4000]; // 指数退避
