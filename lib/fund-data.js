...
async function getFundNavHistory(fundCode, days) {
  ...
  if (cached.length > 0) {
    ...
    // 至少拉 1 页，最多拉满请求天数
    pagesToFetch = Math.max(1, Math.ceil(Math.min(tradingDaysNeeded, days) / pageSize));
  } else {
  ...
  const retryDelays = [2000, 4000, 6000]; // 增加重试间隔
  ...
  for (let page = 1; page <= pagesToFetch; page++) {
    ...
    try {
      ...
      if (!json.Data || !json.Data.LSJZList || json.Data.LSJZList.length === 0) {
        ...
        console.warn("[data] fund " + fundCode + " page " + page + " empty data: " + respInfo);
        fetchSuccess = false;
...
module.exports = { getFundNavHistory: getFundNavHistory, calcIndicators: calcIndicators, getFundPurchaseInfo: getFundPurchaseInfo, getPremiumRate: getPremiumRate, getFundBasicInfo: getFundBasicInfo, getMarketSnapshot: getMarketSnapshot, getMarketNews: getMarketNews, getMarketSentiment: getMarketSentiment, loadNavCache: loadNavCache, cleanStaleCache: cleanStaleCache, initNavDb: initNavDb };
