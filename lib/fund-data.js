...
async function getFundNavHistory(fundCode, days) {
  ...
  var pagesToFetch;
  if (cached.length > 0 && cached.length >= 60) {
    pagesToFetch = 2; // 增量更新（已有足够数据）
  } else if (cached.length > 0) {
    pagesToFetch = Math.ceil(days / pageSize); // 数据不足，全量拉取
  } else {
...
function cleanStaleCache() {
  ...
  var removed = [];
  var kept = 0;
  var staleThreshold = 180 * 86400000; // 180天
  var now = Date.now();
  ...
  for (var i = 0; i < codes.length; i++) {
    ...
    var lastDate = navs[navs.length - 1].date;
...
module.exports = { getFundNavHistory: getFundNavHistory, calcIndicators: calcIndicators, getFundPurchaseInfo: getFundPurchaseInfo, getPremiumRate: getPremiumRate, getFundBasicInfo: getFundBasicInfo, getMarketSnapshot: getMarketSnapshot, getMarketNews: getMarketNews, getMarketSentiment: getMarketSentiment, loadNavCache: loadNavCache, cleanStaleCache: cleanStaleCache };
