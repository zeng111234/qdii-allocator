...
function validRows(rows) {
  ...
  const alreadyIndexed = (rows || []).some(function (row) {
    return row && row.totalReturnBasis === totalReturn.TOTAL_RETURN_BASIS && Number(row.totalReturnIndex) > 0;
  });
  const ordered = alreadyIndexed
    ? (rows || []).filter(function (row) {
      return row && row.date && row.totalReturnBasis === totalReturn.TOTAL_RETURN_BASIS && Number(row.totalReturnIndex) > 0;
    }).map(function (row) {
      return Object.assign({}, row, { nav: Number(row.totalReturnIndex) });
    }).sort(function (left, right) { return left.date.localeCompare(right.date); })
    : totalReturn.buildReinvestedTotalReturnIndex(rows);
  if (Array.isArray(rows)) VALID_ROWS_CACHE.set(rows, ordered);
  VALID_ROWS_CACHE.set(ordered, ordered);
  return ordered;
}

function prepareResearchHistories(navCache, codes, strategyStartDates) {
  const histories = {};
  const coverage = {};
  const starts = strategyStartDates && typeof strategyStartDates === "object" ? strategyStartDates : {};
  (codes || []).forEach(function (code) {
    const raw = ((navCache && navCache[code]) || []).filter(function (row) {
      return row && row.date;
    }).slice().sort(function (left, right) { return left.date.localeCompare(right.date); });
    const totalReturnRows = raw.filter(function (row) {
      return Number(row.nav) > 0 && Number(row.accNav) > 0;
    });
    const strategyStartDate = starts[code] ? String(starts[code]) : null;
    const filtered = strategyStartDate
      ? totalReturnRows.filter(function (row) { return row.date >= strategyStartDate; })
      : totalReturnRows;
    histories[code] = totalReturn.buildReinvestedTotalReturnIndex(filtered);
    coverage[code] = {
      rawStartDate: raw.length ? raw[0].date : null,
      strategyStartDate: strategyStartDate,
      effectiveStartDate: filtered.length ? filtered[0].date : null,
      latestDate: filtered.length ? filtered[filtered.length - 1].date : null,
      rawRows: raw.length,
      usableRows: filtered.length,
      excludedPreStrategyRows: totalReturnRows.length - filtered.length,
      missingAccNavRows: raw.length - totalReturnRows.length,
      totalReturnBasis: totalReturn.TOTAL_RETURN_BASIS,
      totalReturnFormula: totalReturn.TOTAL_RETURN_FORMULA
    };
  });
  return { histories: histories, coverage: coverage };
}
...
function simulateMonthlyContributions(histories, options) {
  ...
  Object.keys(shares).forEach(function (code) {
    ...
    if (row) endValue += shares[code] * Number(row.nav) * (1 - sellFeeRate);
  });
...
module.exports = {
  prepareResearchHistories: prepareResearchHistories,
