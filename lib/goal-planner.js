...
function signedAmount(transaction) {
  ...
  return transaction.type === "SELL" && amount > 0 ? -amount : amount;
}

function confirmedCashFlow(holding) {
  if (holding.confirmedAmount !== null && holding.confirmedAmount !== undefined &&
      Number.isFinite(Number(holding.confirmedAmount))) {
    return Number(holding.confirmedAmount);
  }
  const buys = Array.isArray(holding.buys) ? holding.buys : [];
  if (buys.length > 0) {
    return buys.reduce(function (sum, transaction) {
      const hasShares = Object.prototype.hasOwnProperty.call(transaction, "shares");
      const shares = Number(transaction.shares);
      const isConfirmed = hasShares ? Number.isFinite(shares) && shares !== 0 : Number(transaction.nav) > 0;
      const amount = signedAmount(transaction);
      return isConfirmed && amount !== null ? sum + amount : sum;
    }, 0);
  }
  if (Number(holding.totalShares) > 0 && Number.isFinite(Number(holding.totalAmount))) {
    return Number(holding.totalAmount) - (Number(holding.pendingAmount) || 0);
  }
  return null;
}

function calculateAnnualizedReturn(portfolio, navCache, asOf) {
  if (!portfolio || !Array.isArray(portfolio.holdings)) return null;
  const endDate = asOf instanceof Date ? asOf : new Date(asOf || Date.now());
  const startDate = new Date(portfolio.startDate);
  if (Number.isNaN(endDate.getTime()) || Number.isNaN(startDate.getTime())) return null;

  let totalInvested = 0;
  let totalValue = 0;
  let confirmedHoldings = 0;
  for (let i = 0; i < portfolio.holdings.length; i++) {
    const holding = portfolio.holdings[i];
    const valuation = allocationPolicy.calculateHoldingValuation(
      holding,
      navCache,
      endDate.toISOString().slice(0, 10)
    );
    if (!valuation.valuationComplete) return null;
    if (!valuation.hasConfirmedPosition) continue;
    const cashFlow = confirmedCashFlow(holding);
    if (cashFlow === null) return null;
    totalInvested += cashFlow;
    totalValue += valuation.value;
    confirmedHoldings++;
  }
  if (confirmedHoldings === 0 || !(totalInvested > 0)) return null;

  const daysSinceStart = Math.floor((endDate.getTime() - startDate.getTime()) / 86400000);
  if (daysSinceStart <= 0) return 0;
  const totalReturn = (totalValue / totalInvested - 1) * 100;
  return round2(totalReturn / daysSinceStart * 365);
}
  ...
  for (let i = 0; i < data.goals.length; i++) {
    ...
    switch (goal.metric) {
      case "annualized_return":
        current = calculateAnnualizedReturn(portfolio, navCache, new Date());
        if (current !== null) {
