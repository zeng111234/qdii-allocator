    ...
    switch (goal.metric) {
      case "annualized_return":
        if (portfolio && portfolio.holdings) {
          ...
          for (var h = 0; h < portfolio.holdings.length; h++) {
            var holding = portfolio.holdings[h];
            for (var b = 0; b < holding.buys.length; b++) {
          ...
          if (totalInv > 0) {
            var daysSinceStart = Math.floor((Date.now() - new Date(portfolio.startDate).getTime()) / 86400000);
            var totalReturn = (totalVal / totalInv - 1) * 100;
      ...
      case "dca_streak":
        if (portfolio && portfolio.holdings) {
          var allDates = [];
          for (var hh = 0; hh < portfolio.holdings.length; hh++) {
            for (var bb = 0; bb < portfolio.holdings[hh].buys.length; bb++) {
...
function formatGoalReport() {
  ...
  var active = data.goals.filter(function (g) { return g.status === GOAL_STATUS.ACTIVE; });
  var completed = data.goals.filter(function (g) { return g.status === GOAL_STATUS.COMPLETED; });
  ...
  if (active.length > 0) {
    ...
    for (var i = 0; i < active.length; i++) {
      ...
      var currentStr = g.current !== null ? g.current + g.unit : "N/A";
