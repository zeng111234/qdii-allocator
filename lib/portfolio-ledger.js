...
function derivePortfolio(ledger) {
  ...
  ledger.transactions.forEach(function (tx) {
    if (!holdingsByCode[tx.code]) holdingsByCode[tx.code] = { code: tx.code, buys: [], totalAmount: 0, totalShares: 0 };
  ...
  return {
    holdings: Object.values(holdingsByCode).filter(function (holding) { return holding.totalShares > 0; }),
