...
const { formatLocalDate } = require("./utils");
...
const BUILT_IN_HOLIDAYS = [
  ...
  // 春节休市: 2026-02-15 ~ 2026-02-23（上交所2025-12-22公告）
...
function loadHolidays() {
  ...
  const holidays = new Set(BUILT_IN_HOLIDAYS);

  // 加载用户自定义节假日
  try {
    if (fs.existsSync(HOLIDAYS_FILE)) {
      const userHolidays = JSON.parse(fs.readFileSync(HOLIDAYS_FILE, "utf-8"));
...
/**
 * 格式化日期为 YYYY-MM-DD（复用 utils.formatLocalDate）
 * @param {Date} d
 * @returns {string}
 */
...
function addTradingDays(startDate, tradingDays) {
  ...
  let added = 0;
  let skipped = 0;
  let maxIterations = tradingDays * 3 + 30; // 安全上限，防止死循环
...
function extractAllTradingDays() {
  ...
  try {
    ...
    console.warn("[calendar] 读取全局交易日缓存失败:", e.message);
    return new Set();
  }
}
...
function hasNavData(dateStr) {
  const tradingDays = extractAllTradingDays();
...
function printCalendarInfo() {
  ...
  // 从 nav-cache 提取交易日
  const navTradingDays = extractAllTradingDays();
  if (navTradingDays.size > 0) {
    const navArray = Array.from(navTradingDays).sort();
