...
function verifyGrounding(aiOutput, context) {
  ...
  // CHECK 1: 基金代码验证
  if (context.ranked) {
    const validCodes = new Set(context.ranked.map(function (f) { return f.code; }));
    ...
    const mentionedCodes = aiOutput.match(codePattern) || [];
    for (let i = 0; i < mentionedCodes.length; i++) {
      const code = mentionedCodes[i];
  ...
  const percentages = aiOutput.match(pctPattern) || [];
  let extremeCount = 0;
  for (let j = 0; j < percentages.length; j++) {
    const val = parseFloat(percentages[j]);
    if (Math.abs(val) > 200) {
      extremeCount++;
  ...
  if (context.ranked && context.ranked.length > 0) {
    const topCodes = context.ranked.slice(0, 5).map(function (f) { return f.code; });
    const topNames = context.ranked.slice(0, 5).map(function (f) { return f.name; });
    let recommended = 0;
    for (let k = 0; k < topCodes.length; k++) {
  ...
  // CHECK 4: 风险提示检查
  const riskKeywords = ["风险", "回撤", "波动", "亏损", "注意", "谨慎", "不构成"];
  const hasRiskWarning = riskKeywords.some(function (kw) { return aiOutput.indexOf(kw) >= 0; });
  checks.push({
    type: "risk_warning",
  ...
  const score = checks.length > 0 ? round2((passedCount / checks.length) * 100) : 100;
