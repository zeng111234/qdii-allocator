...
function returnsByDate(navHistory) {
  ...
  for (let i = 1; i < rows.length; i++) {
    result[rows[i].date] = {
      previousDate: rows[i - 1].date,
      value: Number(rows[i].nav) / Number(rows[i - 1].nav) - 1
    };
  }
  return result;
}
...
function alignReturnsByDate(leftHistory, rightHistory, limit) {
  ...
  let dates = Object.keys(left).filter(function (date) {
    return Object.prototype.hasOwnProperty.call(right, date) && left[date].previousDate === right[date].previousDate;
  }).sort();
  if (limit && dates.length > limit) dates = dates.slice(-limit);
  return {
    dates: dates,
    left: dates.map(function (date) { return left[date].value; }),
    right: dates.map(function (date) { return right[date].value; })
  };
}
...
function holdingValue(holding) {
  if (holding.currentValue !== null && holding.currentValue !== undefined && Number.isFinite(Number(holding.currentValue))) {
    return Number(holding.currentValue);
  }
  if (Number.isFinite(Number(holding.confirmedAmount))) return Number(holding.confirmedAmount);
  if (Number(holding.totalShares) > 0 && Number(holding.latestNav) > 0) return Number(holding.totalShares) * Number(holding.latestNav);
  if (Number.isFinite(Number(holding.pendingAmount)) && Number.isFinite(Number(holding.totalAmount))) {
    return Math.max(0, Number(holding.totalAmount) - Number(holding.pendingAmount));
  }
 ...
 * 计算持仓基金间的相关性矩阵
 */
function calcCorrelationMatrix(holdings, days, navCacheOverride) {
  if (!days) days = 60;
  const navCache = navCacheOverride || loadNavCache();
  ...
  for (let j = 0; j < codes.length; j++) {
    ...
    for (let k = 0; k < codes.length; k++) {
      if (j === k) {
        row.push(1);
      } else if (navCache[codes[j]] && navCache[codes[k]]) {
        const aligned = alignReturnsByDate(navCache[codes[j]], navCache[codes[k]], days);
        const corr = pearsonCorrelation(aligned.left, aligned.right);
...
function calcPortfolioRisk(holdings, navCache) {
  ...
  for (let i = 0; i < holdings.length; i++) {
    ...
    const value = h.totalShares > 0 && latestNav ? h.totalShares * latestNav : holdingValue(h);
  ...
  for (let r = 0; r < holdingValues.length; r++) {
    ...
    if (navs && navs.length > 60) {
      allReturns.push({ code: holdingValues[r].code, returns: returnsByDate(navs.slice(-61)), weight: holdingValues[r].weight });
    }
  }

  if (allReturns.length === 0) return null;

  let commonDates = Object.keys(allReturns[0].returns);
  for (let r = 1; r < allReturns.length; r++) {
    commonDates = commonDates.filter(function (date) {
      return allReturns[r].returns[date] &&
        allReturns[r].returns[date].previousDate === allReturns[0].returns[date].previousDate;
    });
  }
  commonDates.sort();
  const includedWeight = allReturns.reduce(function (sum, row) { return sum + row.weight; }, 0);
  const portfolioReturns = commonDates.map(function (date) {
    return allReturns.reduce(function (sum, row) {
      return sum + row.returns[date].value * row.weight / includedWeight;
    }, 0);
  });
  ...
  // 相关性扣分（如果持仓基金高度相关）
  try {
    const corr = calcCorrelationMatrix(holdings, 60, navCache);
    if (corr && corr.matrix) {
      let highCorrCount = 0;
