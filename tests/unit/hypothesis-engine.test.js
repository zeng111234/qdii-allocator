...
function reset() {
  fs.writeFileSync(HYP_FILE, JSON.stringify({ hypotheses: [], stats: { total: 0, validated: 0, invalidated: 0, expired: 0 } }));
}

// Helper: create a hypothesis and return it
function createTestHypothesis(overrides) {
  const defaults = {
    fundCode: '000001',
    fundName: 'Test Fund',
    type: '趋势跟踪',
...
test('createHypothesis creates and saves a hypothesis with correct fields', function () {
  ...
  try {
    ...
    delete require.cache[require.resolve('../../lib/hypothesis-engine')];
    const he = require('../../lib/hypothesis-engine');
    const h = he.createHypothesis(
      '110011', 'Test Fund', '均值回归', 'Fund is oversold',
      { target: 15, stopLoss: -10, timeHorizon: 60 }
    );
...
test('updateHypothesisReturns calculates follow-up returns from navCache', function () {
  ...
  try {
    ...
    delete require.cache[require.resolve('../../lib/hypothesis-engine')];
    const he = require('../../lib/hypothesis-engine');
    const h = he.createHypothesis('000001', 'Fund', '趋势跟踪', 'test', { target: 999, stopLoss: -999, timeHorizon: 999 });
    ...
    const navCache = {
      '000001': [
        { date: dateOffset(0), nav: 1.000 },
        { date: dateOffset(3), nav: 1.030 },
        { date: dateOffset(7), nav: 1.070 },
        { date: dateOffset(14), nav: 1.140 },
        { date: dateOffset(30), nav: 1.300 }
      ]
    };
    ...
    assert.strictEqual(updated.navAtCreation, 1.000);
  } finally {
    restore();
  }
});
...
test('updateHypothesisReturns marks as VALIDATED when return >= target', function () {
  ...
  try {
    ...
    const navCache = {
      '000001': [
        { date: createdDate, nav: 1.000 },
        { date: createdDate, nav: 1.100 }  // 10% return >= 5% target
      ]
    };
...
test('updateHypothesisReturns marks as INVALIDATED when return <= stopLoss', function () {
  ...
  try {
    ...
    delete require.cache[require.resolve('../../lib/hypothesis-engine')];
    const he = require('../../lib/hypothesis-engine');
    const h = he.createHypothesis('000001', 'Fund', '趋势跟踪', 'test', { target: 999, stopLoss: -5, timeHorizon: 999 });
    const createdDate = h.createdAt.substring(0, 10);

    const navCache = {
      '000001': [
        { date: createdDate, nav: 1.000 },
        { date: createdDate, nav: 0.900 }  // -10% return <= -5% stopLoss
      ]
    };
...
test('updateHypothesisReturns marks as EXPIRED when days > timeHorizon', function () {
  ...
  try {
    ...
    delete require.cache[require.resolve('../../lib/hypothesis-engine')];
    const he = require('../../lib/hypothesis-engine');
    const h = he.createHypothesis('000001', 'Fund', '趋势跟踪', 'test', { target: 999, stopLoss: -999, timeHorizon: 5 });
    const createdDate = h.createdAt.substring(0, 10);
...
test('getHypothesisStats returns correct win rate', function () {
  ...
  try {
    ...
    const navCache = {
      '000001': [{ date: createdDate1, nav: 1.000 }, { date: createdDate1, nav: 1.050 }],
      '000002': [{ date: createdDate1, nav: 1.000 }, { date: createdDate1, nav: 0.900 }]
    };
...
test('formatHypothesisReport returns string with stats', function () {
  ...
  try {
    ...
    he.createHypothesis('000001', 'Test Fund', '均值回归', 'Oversold bounce expected', { target: 10, stopLoss: -5, timeHorizon: 30 });
