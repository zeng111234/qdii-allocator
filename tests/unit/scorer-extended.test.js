...
const { scoreFund, rankTopN, applyRotation, WEIGHTS } = require('../../lib/scorer');
...
function makeIndicators(overrides) {
  return Object.assign({
    ...
    maDeviation: 0, drawdown: -5,
    recent5Change: 0, volatility: 1.5,
    longTermTrend: 'neutral', annualizedReturn: 8,
    recent30Change: 0, recent60Change: 0, recent90Change: 0,
    peakDistance: -30,
    navs: [8,8.5,9,9.5,10,10.5,11,11.5,12,12.5,13,13.5,14,14.5,15,15.5,16,16.5,17,17.5,18,18.5,19,19.5,20]
  }, overrides || {});
}
...
function makeFund(overrides) {
  return Object.assign({
    ...
    status: 'active', dailyLimit: 5000, feeRate: 0.8, indexGroup: 'NDX100'
  }, overrides || {});
}
...
test('scoreFund: 回撤越大分数越低', () => {
  ...
  assert.ok(small.score > large.score,
    `小回撤(${small.score})应高于大回撤(${large.score})`);
});
...
test('scoreFund: 低于均线分数更高（逆向买入）', () => {
  ...
  assert.ok(below.score > above.score,
    `低于均线(${below.score})应高于高于均线(${above.score})`);
});
...
test('scoreFund: 近高点扣分，远高点不扣', () => {
  ...
  assert.ok(far.score > near.score,
    `远离高点(${far.score})应高于近高点(${near.score})`);
});
...
test('scoreFund: 极端负面不会出现负分', () => {
  ...
  const indicators = makeIndicators({
    longTermTrend: 'bear', drawdown: -50, maDeviation: 20,
    peakDistance: -1, volatility: 10, recent5Change: -20,
    sharpeRatio: -2, yearReturn: -50, maxDrawdown: -60
  });
