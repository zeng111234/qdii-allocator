...
test("scoreFund: PE < 18 should add WEIGHTS.valuationBonus", () => {
  ...
  assert.ok(result.reason.includes("PE") || result.score > WEIGHTS.base, "Should include valuation bonus");
...
test("scoreFund: PE 25-30 should add WEIGHTS.valuationPenalty * 0.5", () => {
  ...
  const _expectedDiff = WEIGHTS.valuationPenalty * 0.5;
  assert.ok(
    result.score < baseResult.score,
...
test("scoreFund: Nasdaq fund type should use NDX PE", () => {
  ...
  const result = scoreFund(fund, indicators, null, null, null, [], 0, options);
  const baseResult = scoreFund(fund, indicators, null, null, null, [], 0, {});
  assert.ok(result.score > baseResult.score, "Nasdaq fund should use NDX PE (cheap) and get bonus");
});
...
test("scoreFund: S&P500 fund type should use SPX PE", () => {
  ...
  const result = scoreFund(fund, indicators, null, null, null, [], 0, options);
  const baseResult = scoreFund(fund, indicators, null, null, null, [], 0, {});
  assert.ok(result.score > baseResult.score, "S&P500 fund should use SPX PE (cheap) and get bonus");
});
