...
const alt = require('../../lib/alternatives');

// ========== ALTERNATIVES_MAP ==========

test('ALTERNATIVES_MAP - has expected category keys', function () {
  const expected = ['纳指100', '标普500', '港股', '全球精选', '亚太', '德国DAX', '石油', '生物科技', 'REITs'];
...
test('ALTERNATIVES_MAP - each entry has required fields', function () {
  const types = Object.keys(alt.ALTERNATIVES_MAP);
  assert.ok(types.length >= 9);
  for (let i = 0; i < types.length; i++) {
    const alts = alt.ALTERNATIVES_MAP[types[i]];
...
test('POLICY_RISK_LEVELS - each entry has risk/reason/advice', function () {
  const types = Object.keys(alt.POLICY_RISK_LEVELS);
  assert.ok(types.length >= 8);
  for (let i = 0; i < types.length; i++) {
    const level = alt.POLICY_RISK_LEVELS[types[i]];
...
test('getAlternatives - returns alternatives for known type', function () {
  const result = alt.getAlternatives('纳指100');
...
test('getAlternatives - unknown type returns empty', function () {
  const result = alt.getAlternatives('非典型类型');
  assert.strictEqual(result.length, 0);
});

test('getAlternatives - excludes specified fund code', function () {
  const result = alt.getAlternatives('纳指100', '270042');
  const found = result.some(function (a) { return a.code === '270042'; });
...
test('analyzeAlternatives - fund with known type returns suggestions', function () {
  const result = alt.analyzeAlternatives([{ code: '270042', name: '广发纳斯达克100A', type: '纳指100' }]);
...
test('analyzeAlternatives - excludes suspended fund from own alternatives', function () {
  const result = alt.analyzeAlternatives([{ code: '040046', name: '华安纳斯达克100A', type: '纳指100' }]);
  const found = result[0].alternatives.some(function (a) { return a.code === '040046'; });
  assert.strictEqual(found, false);
});
...
test('formatAlternativesReport - with suggestions returns formatted report', function () {
  const report = alt.formatAlternativesReport([{ code: '270042', name: '广发纳斯达克100A', type: '纳指100' }]);
...
test('generatePolicyRiskOverview - empty funds returns header only', function () {
  const result = alt.generatePolicyRiskOverview([]);
...
test('generatePolicyRiskOverview - with active funds shows status', function () {
  ...
  const result = alt.generatePolicyRiskOverview(funds);
