...
// ============ 配置 ============
const RSSHUB_URL = "https://my-rsshub-z6ig.onrender.com";
const TWITTER_HANDLE = "aleabitoreddit";
const CHECK_INTERVAL = 10 * 60 * 1000; // 10 分钟
const SEEN_FILE = path.join(__dirname, "data", "seen-tweets.json");
...
function parseRssItems(xml) {
  ...
  while ((match = itemRe.exec(xml)) && items.length < 20) {
    ...
    const title = titleMatch ? titleMatch[1].replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g, "$1").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">") : "";
    const guid = guidMatch ? guidMatch[1].trim() : "";
    const desc = descMatch ? descMatch[1].replace(/<!\[CDATA\[([\s\S]*?)\]\]>/g, "$1").replace(/<[^>]+>/g, " ").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/\s+/g, " ").trim() : "";
...
function extractTickers(text) {
  const matches = text.match(/\$[A-Z]{2,6}/g) || [];
  const unique = {};
...
async function sendNotification(newTweets) {
  ...
  newTweets.forEach(function(t, i) {
    const tickers = extractTickers(t.title + " " + t.desc);
    lines.push("--- 推文 " + (i+1) + " ---");
    lines.push(t.title);
...
async function checkOnce() {
  ...
  try {
    const url = RSSHUB_URL + "/twitter/user/" + TWITTER_HANDLE;
    const xml = await httpGet(url, 30000);
    ...
    const seen = loadSeen();
    const newTweets = items.filter(function(item) {
